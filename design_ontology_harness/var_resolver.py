"""CSS custom property resolver — recursive var() chain resolution with cycle detection.

Adapted from fivetaku/insane-design var_resolver.py.
"""
...
def parse_all_custom_properties(css: str) -> dict[str, str]:
    """Return {var_name: raw_value} for all --* declarations in CSS."""
    props: dict[str, str] = {}
...
def resolve_var(name: str, props: dict[str, str], seen: set[str] | None = None) -> str | None:
    """Resolve a single CSS var() chain to its terminal value. Returns None if unresolvable or cyclic."""
...
def resolve_all(props: dict[str, str]) -> dict[str, dict]:
    ...
    Returns {var_name: {raw, resolved_terminal, chain: [str]}} for every var.
    """
...
def resolve_css(css: str) -> dict:
    """Pipeline entry point: parse CSS and resolve all var() chains.
    ...
    Returns:
        {
            ...
            "resolved": {var_name: {raw, resolved_terminal, chain}},
        }
    """
