...
def build_fonts_css(font_system: dict, project: str) -> tuple[str, list[str]]:
    ...
    for name in families:
        ...
        if recipe is None or recipe["kind"] == "generic":
            continue
        if recipe["kind"] in ("google-fonts", "cdn-css"):
            imports.append(f'@import url("{recipe["css_url"]}");')
        elif recipe["kind"] == "self-host":
            faces.append(
                "@font-face {\n"
                f'  font-family: "{recipe["family"]}";\n'
                "  font-style: normal;\n"
                f'  font-weight: {recipe["weight_range"]};\n'
                "  font-display: swap;\n"
                f'  src: url("./fonts/{recipe["file"]}") format("woff2-variations");\n'
                "}"
            )
        else:
            manual.append(name)
    ...
    if imports:
        lines.append("/* Google Fonts */")
        lines.extend(imports)
        lines.append("")
    if faces:
        lines.append("/* self-host — fonts/fetch-webfonts.mjs 로 내려받습니다. */")
        lines.extend(faces)
        lines.append("")
    if manual:
        lines.append(
        ...
        lines.append("")
    if not imports and not faces and not manual:
...
def _fetch_webfonts_script(entries: list[dict]) -> str:
    targets = ",\n".join(
        f'  {{ url: "{e["woff2_url"]}", file: "{e["file"]}" }}' for e in entries
    )
    return (
...
def _emit_font_loading(artifact_dir: Path, font_system: dict, project: str) -> None:
    ...
    self_hosted = [
        recipe
        for recipe in (
            webfont_recipe((font_system.get(slot) or {}).get("name") if isinstance(font_system.get(slot), dict) else None)
            for slot in ("display", "heading", "body", "korean", "mono")
        )
        if recipe and recipe["kind"] == "self-host"
    ]
    seen: dict[str, dict] = {}
    for recipe in self_hosted:
        seen.setdefault(recipe["file"], recipe)
    ...
    script_path.write_text(_fetch_webfonts_script(list(seen.values())), encoding="utf-8")
