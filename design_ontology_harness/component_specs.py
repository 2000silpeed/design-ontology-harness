...
COMPONENT_ANATOMY: dict[str, dict] = {
    ...
    "surface": {
        "parts": ["container", "header(optional)", "content", "footer(optional)", "action(optional)"],
    ...
    "layout": {
        "parts": ["root", "region", "slot", "responsive-boundary"],
        "states": ["default", "responsive", "collapsed"],
        "accessibility": [
            "landmark는 header/nav/main/aside/footer 중 실제 의미에 맞게 사용",
            "grid/flex 자식은 320px viewport에서 overflow-x를 만들지 않음",
            "스킵 링크와 논리적 focus order를 유지",
        ],
        "tokens": {
            ...
            "border": "var(--color-border)",
            "gap": "var(--space-16)",
            "section-padding": "var(--space-32) var(--space-24)",
            "container-max-width": "1120px",
        },
    },
    "content": {
        "parts": ["container", "media(optional)", "title(optional)", "body", "metadata(optional)"],
        "states": ["default", "hover", "selected"],
        "accessibility": [
            "텍스트 콘텐츠는 의미 있는 HTML 태그를 우선",
            "코드/키보드 입력 표시는 스크린 리더가 읽을 수 있는 라벨과 함께 제공",
            "이미지/썸네일은 alt 정책을 명시",
        ],
        "tokens": {
            "text": "var(--color-text)",
            "text-muted": "var(--color-text-muted)",
            "font": "var(--font-body) / var(--text-sm) / regular",
            "mono": "var(--font-mono)",
            "gap": "var(--space-8)",
        },
    },
