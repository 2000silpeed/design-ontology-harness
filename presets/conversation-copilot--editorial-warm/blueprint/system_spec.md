...
- **Heading note**: editorial serif heading — Lora (signal-desk) 와 차별화, 드래프트 아티팩트 제목 / 스레드 헤더 / 아티클 heading, calm editorial pairing
- **Body note**: prompt composer / chat message body / artifact prose 본문 공용, line-height 1.6–1.7 (reading-first)
...
- **Loading**: Inter(preload), Source Serif Pro(preload), Pretendard(preload), JetBrains Mono(lazy) | display: swap
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=129.55): masthead_energy=Pure Red, cover_signal=Scarlet, paper_field=Buttercream, feature_frame=Ultra Violet, attention_flash=Illuminating
  - ontology-quiet-surface-2 (Quiet Surface, score=77.2): masthead_energy=Pure Orange, cover_signal=Tangerine, paper_field=Flax, feature_frame=Byzantium, attention_flash=Amber
  - ontology-clear-structure-3 (Clear Structure, score=80.8): masthead_energy=Scarlet, cover_signal=Burnt Orange, paper_field=Apricot, feature_frame=Aubergine, attention_flash=Illuminating
  - ontology-strong-signal-4 (Strong Signal, score=87.2): masthead_energy=Grenadine, cover_signal=Amber, paper_field=Apricot, feature_frame=Ultra Violet, attention_flash=Chili Oil
  - ontology-cross-family-5 (Cross Family, score=90.9): masthead_energy=Ruby, cover_signal=Amber, paper_field=Buttercream, feature_frame=Grape Compote, attention_flash=Pure Orange
- **Selection rules**:
...
- **Matched color keywords**:
  - `primary` -> Marsala #964F4C / red.pantone_trend / mood=성숙함, 안정감, 관능적 / axes=earth_or_wine_bias
  - `accent` -> Moss Green #8A9A5B / green.natural / mood=자연적, 차분함, 안정적인 / axes=low_value, low_chroma, earth_or_wine_bias
  - `surface_tint` -> Flax #EEDC82 / yellow.natural / mood=절제, 온기, 내추럴 / axes=mid_value, low_chroma, warm_bias
...
- **Density**: airy (confidence 0.4, provenance inferred) / editorial x3.1500000000000004, hero x0.15, calm x2.6500000000000004
- **Surface Style**: tinted (confidence 0.34, provenance inferred) / warm x3.45, editorial x3.1500000000000004, cream x0.8
- **Corner Style**: medium (confidence 0.18, provenance inferred) / card x1.8
- **Typography Mood**: editorial (confidence 0.47, provenance inferred) / editorial x3.1500000000000004, serif x1.75
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Conversation side panel**: confidence 0.52 / provenance inferred / chat x2.55, assistant x2.85, message x3.6, thread x4.35
- **Editorial feed**: confidence 0.41 / provenance inferred / editorial x3.1500000000000004, feed x0.15
- **Split-pane workspace**: confidence 0.35 / provenance inferred / sidebar x1.8, app x0.45, editor x0.9
- **Data review surface**: confidence 0.2 / provenance inferred / timeline x2.25
...
- **Flow coverage**:
  - document: covered (6 context cards; lazyweb)
  - messaging: covered (1 context cards; lazyweb)
...
- **Context cards**:
  - `research-query-01` (research-query, planned): lex page writing copilot / flows: document / morphology: general-interface-composition
  - `research-query-02` (research-query, planned): jasper ai editor calm / flows: document / morphology: general-interface-composition
  - `research-query-03` (research-query, planned): sudowrite writing assistant / flows: document, messaging / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): notion ai writing panel / flows: document / morphology: card-stack
  - `research-query-05` (research-query, planned): ulysses writing app editorial / flows: document / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, copilot-artifact, copilot-chat, data-display, document, editorial, feedback, input, layout, marketing, navigation, workflow, overlay, social
...
- **diff-viewer** (document, score 18): AI rewrites, policy edits, or reviewer changes need auditability; users must approve what changed before publishing / pairs with: redline-viewer, revision-timeline, approval-rail
- **source-card** (copilot-artifact, score 18): AI output depends on external or internal source records; users need a repeatable citation preview component / pairs with: citation-drawer, evidence-graph, inline-citation
- **resizable-split-pane** (layout, score 17): primary work happens between list, canvas/chat, and detail panels; users need to compare or inspect adjacent information without navigation / pairs with: thread-list, artifact-preview-panel, inspector-drawer
- **citation-drawer** (copilot-artifact, score 16): answers must show supporting policy, document, or source records; users need to inspect evidence without losing conversation context / pairs with: inline-citation, source-card, evidence-graph
- **redline-viewer** (document, score 13): legal, compliance, or editorial text needs reviewer markup; comments must stay anchored to exact text ranges / pairs with: diff-viewer, comment-thread, approval-rail
...
- **command-palette** (overlay, score 10): the product has many actions or navigation targets; expert users benefit from quick action search / pairs with: shortcut-hint, saved-view-bar, filter-builder
- **inspector-drawer** (overlay, score 10): a selected item needs rich detail without leaving the main workflow; users need source facts, owners, versions, or retention metadata / pairs with: policy-matrix, citation-drawer, decision-record-card

- **Visual-reference archetypes**:

- **Conversation sidecar** (overlay / 0.48): chat-panel, message-thread, message-composer, context-drawer
...
- **button**: primary-button, secondary-button, icon-button, regenerate-button, stop-generation-button, new-thread-button, form-actions, player-controls
...
- **data-display**: chat-message, chat-thread, comment-thread, tag, calendar-grid, video-player, chart-container, chart-legend
...
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, empty-conversation-state, status-badge, banner
- **input**: text-field, select, checkbox, switch, segmented-control, tone-slider, prompt-composer, chat-input
...
- **navigation**: breadcrumbs, tabs, pagination, prev-next-pager, app-shell, sidebar-nav, topbar, breadcrumb
- **workflow**: approval-rail
- **overlay**: dialog, popover, tooltip, mention-popup, chart-tooltip, confirm-dialog, autocomplete, tooltip-guide
...
|-----------|-------------|
| app-shell | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| artifact-preview-panel | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| audit-timeline | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| autocomplete | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| avatar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| avatar-cluster | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| badge | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| block-controls | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| breadcrumb | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| calendar-grid | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| chart-container | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-legend | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-tooltip | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chat-input | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| checkbox | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| citation-drawer | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| citation-footnote | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| code-block | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| column-header | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| command-palette | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-input | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-thread | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| confirm-dialog | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| copy-code-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| customer-logo | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| data-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| date-picker | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| date-range-picker | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| decision-record-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| dialog | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| empty-conversation-state | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-feed-illustration | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-state | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| evidence-graph | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| feed-item | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| filter-chip | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-panel | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| follow-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-column | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-legal | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-link | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-social | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footnote | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| form-actions | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| gentle-toast | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| heading-anchor | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| icon-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-alert | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-citation | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inline-code | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inline-format-menu | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inspector-drawer | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-board | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| kanban-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-column | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| language-tab | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| link-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| logo-cloud | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mention-chip | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| mention-popup | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| message-artifact | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| metric-highlight | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-strip | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| new-thread-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| operational-rail | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| outline-sidebar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| pagination | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| paragraph-block | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| player-controls | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| popover | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| post-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| press-quote | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| prev-next-pager | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| primary-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| profile-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| prompt-composer | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| prose-block | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| quote-block | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| radio-group | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| regenerate-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| reply-composer | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| resizable-split-pane | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| reviewer-assignment-picker | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
...
| search-field | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-results | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| secondary-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-header | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segmented-control | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| share-sheet | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sidebar-nav | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-footer | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| slash-command-menu | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| soft-dialog | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-ledger | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| status-badge | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-summary-row | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| step-progress | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| stop-generation-button | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| streaming-cursor | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| suggestion-card | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| switch | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| table-of-contents | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| tabs | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| text-field | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| time-picker | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| timeline-stream | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| toast | `color.Flax→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tone-slider | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip-guide | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| typing-indicator | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| user-menu | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| video-player | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| volume-slider | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| wizard-layout | `color.Marsala→surface`, `color.Moss Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | breadcrumbs, decision-record-card, kanban-card, link-card, player-controls, post-card | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | breadcrumbs, decision-record-card, kanban-card, link-card, player-controls, post-card | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | article-body, block-controls, editor-canvas, editor-toolbar, inline-format-menu, press-quote | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | article-body, block-controls, editor-canvas, editor-toolbar, inline-format-menu, press-quote | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Hero image | generated | high-fidelity-raster-support; SVG: False | Codex image_gen skill | marketing | `public/generated/design-system/manifest.json` | no API fallback |
| Hero image sourced fallback | sourced | high-fidelity-raster-support; SVG: False | Openverse, Wikimedia Commons, Unsplash, Pexels | marketing | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| jasper ai editor calm | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
| lex page writing copilot | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
| notion ai writing panel | Lazyweb MCP real-app corpus | planned | flows: document; morphology: card-stack |
| substack editor warm | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
| sudowrite writing assistant | Lazyweb MCP real-app corpus | planned | flows: document, messaging; morphology: general-interface-composition |
| ulysses writing app editorial | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
