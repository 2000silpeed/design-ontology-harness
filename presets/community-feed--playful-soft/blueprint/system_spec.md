...
- **Heading note**: rounded sans, playful-soft 시각 성격 핵심 — geometric sans/serif 금지
- **Body note**: UI 본문·피드 카드 공용, Nunito 와 어울리는 중립 sans
...
- **Expanded supporting colors**:
  ...
  - Rose Quartz #F7CAC9 / Pastel Reds / reference-color / Rose Quartz stays inside the preferred families.; Rose Quartz reinforces the brand mood signals.
...
- **Combination lists**:
  ...
  - Support Spectrum: support=Peach Puff, support=Salmon, support=Creamsicle, support=Rose Quartz, support=Dark Salmon, support=Buttercream
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=131.8): masthead_energy=Pure Red, cover_signal=Scarlet, paper_field=Buttercream, feature_frame=Ultra Violet, attention_flash=Illuminating
  - ontology-quiet-surface-2 (Quiet Surface, score=80.4): masthead_energy=Salmon, cover_signal=Cornsilk, paper_field=Creamsicle, feature_frame=Midnight Violet, attention_flash=Grenadine
  - ontology-clear-structure-3 (Clear Structure, score=82.9): masthead_energy=Salmon, cover_signal=Byzantium, paper_field=Creamsicle, feature_frame=Very Peri, attention_flash=Illuminating
  - ontology-strong-signal-4 (Strong Signal, score=86.2): masthead_energy=Scarlet, cover_signal=Honey Gold, paper_field=Salmon, feature_frame=Ultra Violet, attention_flash=Chili Oil
  - ontology-cross-family-5 (Cross Family, score=94.6): masthead_energy=Pure Orange, cover_signal=Salmon, paper_field=Buttercream, feature_frame=Periwinkle, attention_flash=Super Sonic
- **Selection rules**:
...
- **Matched color keywords**:
  - `primary` -> Coral Blush #F88379 / orange.pastel / mood=감성적, 따뜻함, 세련됨 / axes=high_value, mid_chroma, low_chroma
  - `accent` -> Mint Green #98FF98 / green.pastel / mood=청량함, 신선함, 현대적 감성 / axes=high_value, low_chroma, cool_bias
...
- **Ontology keyword recommendations**:
  ...
  - Peach Puff #FFDAB9 / orange.pastel / matches 'pastel'; matches 'orange'
  - Creamsicle #FFD7A0 / orange.pastel / matches 'pastel'; matches 'orange'
  - Apricot #FFB27F / orange.natural / matches 'orange'; matches '자연스러움'
  - Buttercream #F3E5AB / yellow.pastel / matches 'pastel'; matches 'warm'
...
- **Density**: airy (confidence 0.15, provenance inferred) / editorial x0.2, hero x0.15
- **Surface Style**: tinted (confidence 0.23, provenance inferred) / warm x1.0, editorial x0.2, soft x3.0500000000000003
- **Corner Style**: round (confidence 0.48, provenance inferred) / rounded x2.45, soft x3.0500000000000003
- **Typography Mood**: playful (confidence 0.25, provenance inferred) / playful x1.2, friendly x1.55
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Editorial feed**: confidence 0.48 / provenance inferred / editorial x0.2, feed x5.550000000000001, magazine x0.25
- **Conversation side panel**: confidence 0.28 / provenance inferred / thread x2.55
- **Data review surface**: confidence 0.2 / provenance inferred / table x0.15, grid x0.15, data x0.15, timeline x1.35
...
- **Context cards**:
  - `research-query-01` (research-query, planned): threads social feed / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-02` (research-query, planned): bluesky timeline / flows: general-product-ui / morphology: timeline
  - `research-query-03` (research-query, planned): mastodon community feed / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): tumblr playful feed / flows: general-product-ui / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, data-display, document, editorial, feedback, input, layout, navigation, overlay, social
...
- **audit-timeline** (data-display, score 2): regulated workflows require traceable user and AI actions; reviewers need to reconstruct what happened before approval / pairs with: decision-record-card, approval-rail, tool-call-trace
...
- **resizable-split-pane** (layout, score 2): primary work happens between list, canvas/chat, and detail panels; users need to compare or inspect adjacent information without navigation / pairs with: thread-list, artifact-preview-panel, inspector-drawer
...
- **reviewer-assignment-picker** (input, score 1): workflows require human approval or reassignment; reviewer choice depends on policy ownership or availability / pairs with: approval-rail, exception-queue, presence-indicator

- **Visual-reference archetypes**:

- No visual-reference archetypes suggested.

- **button**: primary-button, secondary-button, icon-button, follow-button, form-actions
- **data-display**: tag-pill, comment-thread, notification-item, mention-highlight, tag, avatar, profile-card, search-results
- **document**: redline-viewer
- **editorial**: editor-canvas, editor-toolbar, inline-format-menu, slash-command-menu, block-controls
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, empty-feed-illustration, gentle-toast, banner
- **input**: text-field, select, checkbox, switch, segmented-control, reply-composer, comment-input, chip
...
- **overlay**: dialog, popover, tooltip, share-sheet, soft-dialog, mention-popup, notification-center, user-menu
...
|-----------|-------------|
| app-shell | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| audit-timeline | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| autocomplete | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| avatar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| avatar-cluster | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| badge | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| block-controls | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| breadcrumb | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| calendar-grid | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| chat-input | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| checkbox | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| column-header | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| command-palette | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| command-result-item | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-input | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-thread | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| confirm-dialog | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| data-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| date-picker | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| date-range-picker | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| dialog | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| empty-feed-illustration | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-state | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| filter-chip | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-panel | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| follow-button | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-actions | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| gentle-toast | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| icon-button | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-alert | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| kanban-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-column | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| mention-highlight | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| mention-popup | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-strip | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| notification-center | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| notification-item | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| operational-rail | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| pagination | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| popover | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| post-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| presence-indicator | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| primary-button | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| profile-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| radio-group | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| reply-composer | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| resizable-split-pane | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| retention-indicator | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| reviewer-assignment-picker | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| row-actions | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| saved-view-bar | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-field | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-results | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| secondary-button | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-header | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segmented-control | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| share-sheet | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| shortcut-hint | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sidebar-nav | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| slash-command-menu | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| soft-dialog | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-ledger | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| status-badge | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-summary-row | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| switch | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tabs | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| text-field | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| thread-view | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| time-picker | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| timeline-stream | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| toast | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| user-menu | `color.Coral Blush→surface`, `color.Mint Green→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | breadcrumbs, kanban-card, post-card, profile-card, switch, data-display | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | breadcrumbs, kanban-card, post-card, profile-card, switch, data-display | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Comic panel preview | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Codex image_gen skill | filter-panel, metric-strip, tabs | `public/generated/design-system/manifest.json` | no API fallback |
| Comic panel preview sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Openverse, Wikimedia Commons, Unsplash, Pexels | filter-panel, metric-strip, tabs | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | block-controls, editor-canvas, editor-toolbar, inline-format-menu, slash-command-menu, editorial | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | block-controls, editor-canvas, editor-toolbar, inline-format-menu, slash-command-menu, editorial | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Hero image | generated | high-fidelity-raster-support; SVG: False | Codex image_gen skill | feature-comparison | `public/generated/design-system/manifest.json` | no API fallback |
| Hero image sourced fallback | sourced | high-fidelity-raster-support; SVG: False | Openverse, Wikimedia Commons, Unsplash, Pexels | feature-comparison | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| bluesky timeline | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: timeline |
| mastodon community feed | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| threads social feed | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| tumblr playful feed | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
