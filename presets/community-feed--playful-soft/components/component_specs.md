# Bloom Component Specs

총 96개 컴포넌트 | 패밀리: button, data-display, document, editorial, feedback, input, layout, navigation, overlay, social
...
- **card_elevation_tendency**: 카드는 낮은 elevation만 허용하고 색면 차이로 그룹을 구분한다. 내부 여백은 넉넉하게 두고 card breathing room을 확보한다. (source=cards; confidence=0.13; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 넓은 내부 여백과 강한 section breathing room. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=airy, corner=round)
- **border_vs_fill_emphasis**: fill 중심이다. tint surface로 성격을 만들고 border는 아주 얇게 보조한다. (source=cards; confidence=0.13; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 넓은 내부 여백과 강한 section breathing room. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=airy, corner=round)
...
- **filter_nav_density**: filter/nav density는 balanced다. global nav와 local filter를 섞지 말고 계층별 간격 차이로 구조를 드러낸다. (source=navigation; confidence=0.26; provenance=inferred; direction=navigation은 정보 구조를 안정적으로 고정하고 시각적 장식보다 위치 신호를 우선한다.; evidence=Editorial feed, layout=editorial-feed, density=airy)
...
- **operational_surface_role**: 주 작업 표면은 card wall이 아니라 table/list/rail/canvas 계열로 설계한다. 행 간격은 읽기 편하게 두되 반복 항목은 같은 카드 껍질로 감싸지 않는다. (source=data_display; confidence=0.17; provenance=inferred; direction=정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다.; evidence=layout=data-review-surface, density=airy, layout=editorial-feed)
- **filter_nav_density**: filter/nav density는 balanced다. global nav와 local filter를 섞지 말고 계층별 간격 차이로 구조를 드러낸다. (source=navigation; confidence=0.26; provenance=inferred; direction=navigation은 정보 구조를 안정적으로 고정하고 시각적 장식보다 위치 신호를 우선한다.; evidence=Editorial feed, layout=editorial-feed, density=airy)
- **chart_panel_framing**: 차트 패널은 soft tint frame과 restrained divider로 프레이밍한다. 헤더 메타데이터와 본문 visualization 사이의 breathing room을 충분히 둔다. (source=data_display; confidence=0.17; provenance=inferred; direction=정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다.; evidence=layout=data-review-surface, density=airy, layout=editorial-feed, surface=tinted)
