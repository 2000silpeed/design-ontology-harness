# Lattice-Dash System Style Capsule

- preset: `dashboard--bold-confident`
- brand: Lattice-Dash
- mode: `dashboard` / `bold-confident`
...
- brand keywords: `dashboard`, `admin`, `console`, `startup`, `saas`, `b2c`, `growth`, `activation`, `retention`, `cohort`, +6 more
- tone: `confident`, `energetic`, `direct`, `data-forward`, `youthful`
- visual cues: `high-contrast headline`, `vivid ultra violet primary`, `illuminating yellow accent`, `saturated KPI card`, `dense data table`, `impact metric typography`, `youthful startup`, `admin-first bold`, +2 more
- avoid: `editorial-warm`, `magazine-serif`, `minimal-only`, `playful-pastel`, `corporate-conservative`, `document-heavy`, `commerce-checkout`

## Color Roles
| Role | Token | Value | Source | Use |
| --- | --- | --- | --- | --- |
| `primary` | `--ds-color-primary` | `#5F4B8B` | Ultra Violet | 중명도, 중고채도, 파란빛이 섞인 깊고 복합적인 퍼플 / 창의적, 신비감, 미래적, 실험적 |
| `accent` | `--ds-color-accent` | `#F5DF4D` | Illuminating | 고명도, 중채도, 선명하면서 부드러운 라이트 옐로 톤 / 긍정적, 활기찬, 낙관적, 희망적인, 감각적인 |
| `surface_tint` | `--ds-color-surface-tint` | `#FFD7A0` | Creamsicle | 고명도, 저채도, 크리미하고 부드러운 오렌지 계열 / 부드러움, 경쾌함, 청량함, 달콤함 |
...
| `border` | `--ds-color-border` | `#CFC6BA` | Pairing #CFC6BA | Seed pairing support |
| `border_strong` | `--ds-color-border-strong` | `#CFC6BA` | Pairing #CFC6BA | Seed pairing support |
| `ink` | `--ds-color-ink` | `#111111` | Ink | Generated fallback support color |
| `ink_muted` | `--ds-color-ink-muted` | `#5F4B8B` | Ultra Violet | 중명도, 중고채도, 파란빛이 섞인 깊고 복합적인 퍼플 / 창의적, 신비감, 미래적, 실험적 |
...
| `warning` | `--ds-color-warning` | `#F5DF4D` | Illuminating | 고명도, 중채도, 선명하면서 부드러운 라이트 옐로 톤 / 긍정적, 활기찬, 낙관적, 희망적인, 감각적인 |
| `danger` | `--ds-color-danger` | `#5F4B8B` | Ultra Violet | 중명도, 중고채도, 파란빛이 섞인 깊고 복합적인 퍼플 / 창의적, 신비감, 미래적, 실험적 |
| `link` | `--ds-color-link` | `#5F4B8B` | Ultra Violet | 중명도, 중고채도, 파란빛이 섞인 깊고 복합적인 퍼플 / 창의적, 신비감, 미래적, 실험적 |
...
| --- | --- | --- | --- |
| heading | Space Grotesk | `--ds-font-heading` | geometric sans with impact, workspace header / KPI metric headline / activation-funnel stage labe... |
| body | Inter | `--ds-font-body` | dense data table cell / filter chip / ticket row / referral copy 공용, line-height 1.4–1.5 (dashboa... |
...
| --- | --- | --- | --- |
| button | high | `default`, `hover`, `active`, `disabled`, `loading` | `primary-button`, `secondary-button`, `icon-button`, `form-actions`, `hero-cta-group`, `follow-button`, +2 more |
...
| document | high | `default`, `selected`, `commenting`, `resolved` | `callout`, `admonition-block` |
| feedback | high | `info`, `success`, `warning`, `danger`, `loading` | `badge`, `inline-alert`, `empty-state`, `toast`, `status-dot`, `status-badge`, +7 more |
| input | high | `default`, `focus`, `error`, `disabled`, `selected` | `text-field`, `select`, `checkbox`, `switch`, `segmented-control`, `segment-filter`, +19 more |
...
| `activation-funnel` | dashboard-growth | parts: container, header, content-area, footer(optional), action(optional); states: default, loading, empty, error | slots: surface, text, border, radius, padding |
| `goal-tracker` | dashboard-growth | parts: container, header, content-area, footer(optional), action(optional); states: default, loading, empty, error | slots: surface, text, border, radius, padding |
...
| `retention-chart` | dashboard-growth | parts: container, header, content-area, footer(optional), action(optional); states: default, loading, empty, error | slots: surface, text, border, radius, padding |
| `referral-widget` | dashboard-growth | parts: container, header, content-area, footer(optional), action(optional); states: default, loading, empty, error | slots: surface, text, border, radius, padding |
...
- flow coverage: general-product-ui(covered), dashboard(covered), settings(covered), data-review(gap), navigation(gap)
| Context | Provider | Allowed Use |
| --- | --- | --- |
| retool admin console | `lazyweb` | morphology: `general-interface-composition`; flows: `settings` |
| appsmith internal tool | `lazyweb` | morphology: `general-interface-composition`; flows: `general-product-ui` |
| stripe radar dashboard | `lazyweb` | morphology: `general-interface-composition`; flows: `dashboard` |
| posthog activation funnel | `lazyweb` | morphology: `general-interface-composition`; flows: `general-product-ui` |
