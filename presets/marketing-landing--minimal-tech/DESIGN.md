# Beacon System Style Capsule

- preset: `marketing-landing--minimal-tech`
- brand: Beacon
- mode: `marketing-landing` / `minimal-tech`
...
- brand keywords: `marketing-landing`, `landing`, `saas-landing`, `hero`, `pricing`, `feature-grid`, `social-proof`, `testimonial`, `faq`, `cta-section`, +4 more
- tone: `clean`, `neutral`, `precise`, `trustworthy`, `restrained`
- visual cues: `neutral hero surface`, `hairline borders`, `monochrome + restrained accent`, `thin pricing card`, `compact feature grid`, `logo cloud trust strip`, `muted testimonial card`, `geometric sans hierarchy`, +1 more
- avoid: `bold-confident`, `saturated`, `magazine-serif`, `playful`, `decorative`, `ornamental`, `dashboard-heavy`, `warm-editorial`

## Color Roles
| Role | Token | Value | Source | Use |
| --- | --- | --- | --- | --- |
| `primary` | `--ds-color-primary` | `#01889F` | Teal Blue | 중명도, 중채도, 쿨톤의 감각적인 청록색 계열 / 개방감, 에너지, 명료함, 신선함, 감각적 |
| `accent` | `--ds-color-accent` | `#DAA520` | Goldenrod | 중명도, 중채도, 옐로 오렌지 계열 / 따뜻함, 안정감, 고급스러움, 빈티지 감성 |
...
| `border_strong` | `--ds-color-border-strong` | `#C4C4C4` | Pairing #C4C4C4 | Seed pairing support |
| `ink` | `--ds-color-ink` | `#333333` | Pairing #333333 | Seed pairing support |
| `ink_muted` | `--ds-color-ink-muted` | `#6B705C` | Pairing #6B705C | Seed pairing support |
| `ink_subtle` | `--ds-color-ink-subtle` | `#7A9EAF` | Pairing #7A9EAF | Seed pairing support |
...
| `link` | `--ds-color-link` | `#01889F` | Teal Blue | 중명도, 중채도, 쿨톤의 감각적인 청록색 계열 / 개방감, 에너지, 명료함, 신선함, 감각적 |
...
| --- | --- | --- | --- |
| heading | Inter | `--ds-font-heading` | geometric sans, hero headline / feature title / pricing plan name — serif 금지 |
| body | Inter | `--ds-font-body` | hero subheadline / feature description / testimonial quote / faq answer 본문 공용 |
...
| --- | --- | --- | --- |
| button | high | `default`, `hover`, `active`, `disabled`, `loading` | `primary-button`, `secondary-button`, `icon-button`, `hero-cta-group`, `cta-button-group`, `form-actions`, +3 more |
| data-display | high | `default`, `sorted`, `filtered`, `empty`, `loading` | `pricing-card`, `feature-comparison`, `metric-strip`, `status-summary-row`, `task-surface-header`, `source-ledger`, +16 more |
...
| feedback | high | `info`, `success`, `warning`, `danger`, `loading` | `badge`, `inline-alert`, `empty-state`, `toast`, `status-dot`, `upgrade-banner`, +4 more |
| input | high | `default`, `focus`, `error`, `disabled`, `selected` | `text-field`, `select`, `checkbox`, `switch`, `segmented-control`, `filter-chip`, +7 more |
| marketing | high | `default`, `hover`, `in-view` | `faq-section`, `faq-item`, `faq-question`, `faq-answer`, `hero-container`, `hero-eyebrow`, +29 more |
...
| `cta-section` | marketing | parts: section-container, headline, supporting-text, button-group; states: default | slots: surface, text, radius, padding |
...
- flow coverage: general-product-ui(covered), pricing(covered), dashboard(gap), data-review(gap)
| Context | Provider | Allowed Use |
| --- | --- | --- |
| linear landing hero | `lazyweb` | morphology: `general-interface-composition`; flows: `general-product-ui` |
| vercel landing marketing | `lazyweb` | morphology: `general-interface-composition`; flows: `general-product-ui` |
| stripe pricing page | `lazyweb` | morphology: `general-interface-composition`; flows: `pricing` |
| railway landing | `lazyweb` | morphology: `general-interface-composition`; flows: `general-product-ui` |
