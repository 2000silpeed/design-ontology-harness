# EKOS Knowledge Intake Design System Spec

## 1. Positioning

- **Brand**: EKOS Knowledge Intake
- **Product**: SAP 업무 지식을 아는 현업 사용자가 온톨로지 용어 없이 문서·설명을 지식으로 정리하고, 시스템이 이해한 사실을 고치고, 근거를 확인해 승인하는 데스크톱 웹 지식 등록·검토 워크벤치
- **Audience**: 현업 지식 제공자 (SAP 운영 담당자, 온톨로지 비전문가), 지식 검토자 (팀 리드, 승인 권한자), 운영 관리자 (입력 큐·검증 실패·연결 시스템 관리)
- **Platforms**: web, desktop-web
- **Accessibility floor**: WCAG 2.2 AA, full keyboard completion of all core tasks, list/table alternative for canvas graph, non-color status encoding (color + icon + text), focus and state changes announced to assistive tech, prefers-reduced-motion respected, body text 14-16px, long copy at 65-75ch measure

### Application Concept

- **Primary job**: 현업 사용자가 문서나 업무 설명을 넣고, 시스템이 이해한 사실 초안을 고친 뒤, 근거를 확인해 승인까지 끝낸다
- **Operating mode**: review
- **Success moment**: 검토 가능한 사실이 반영 범위·개수와 함께 승인되고, 지식 상세에서 원문·근거·승인자가 한 화면에 보인다
- **Domain objects**:
- 지식 초안 (추출된 사실)
- 원문 (문서·화면·코드 발췌)
- 근거 (원문 위치·매칭 키·추론 규칙·수정 이력)
- 관계 후보 (업무 문장으로 번역된 연결)
- 새 개념 제안
- 승인 이력과 지식 상태
- **Differentiation intent**:
- 그래프가 아니라 업무 문장이 1급 시민이다 — 모든 지식은 먼저 한 문장으로 보인다
- 내부 어휘(ontology/fragment/node/edge/SHACL)는 화면에서 번역되고, 원문은 고급 서랍에만 있다
- 승인 버튼은 항상 반영 범위와 개수를 옆에 달고, 불가 항목은 이유와 해결 행동을 보여준다
- 원문은 불변 근거로 보존되고, 사용자는 언제든 원문과 구조화 문장을 나란히 대조할 수 있다
- **Must feel different from**:
- generic SaaS dashboard
- graph database admin console (Neo4j-style)
- developer ontology editor (Protégé-style)
- template card wall
- **Signature structural moves**:
- sentence-first fact row: 모든 지식 항목은 상태 태그가 붙은 한 문장으로 시작하고, 구조 필드는 그 아래에 종속된다
- evidence side panel: 근거(원문 발췌, 매칭 키, 추론 규칙, 수정 이력)가 항상 한 제스처 거리에 있는 접이식 패널
- translated-vocabulary chrome: 내부 기술 어휘는 UI 어디에도 없고, 고급 서랍(advanced drawer)에서만 원문 노출
- scoped approval bar: 승인 행동에 반영 범위·개수·불가 사유가 상시 병기되는 하단 액션 바
- 그래프가 아니라 업무 문장이 1급 시민이다 — 모든 지식은 먼저 한 문장으로 보인다
- 내부 어휘(ontology/fragment/node/edge/SHACL)는 화면에서 번역되고, 원문은 고급 서랍에만 있다
- 승인 버튼은 항상 반영 범위와 개수를 옆에 달고, 불가 항목은 이유와 해결 행동을 보여준다
- 원문은 불변 근거로 보존되고, 사용자는 언제든 원문과 구조화 문장을 나란히 대조할 수 있다
- **Repetition risks**:
- 검토 화면이 표 중심 admin CRUD로 회귀
- 관계 후보를 숫자 점수 배지로만 표시
- 진행 상태를 스피너 하나로 축약
- 아카이브 감성을 위해 종이 질감·장식 테두리를 본문 위에 얹어 가독성 훼손
- 상태 색상을 텍스트·아이콘 없이 색만으로 구분
- **Anti-convergence rule**: If two generated products share the same first-screen composition, navigation model, component hierarchy, and density, the skeleton is under-specified.

### Layout Skeleton

- **Composition**: split-review-workbench: 좌측 사실 목록(상태 태그 + 필터), 우측 선택 사실의 문장 편집 + 접이식 근거 패널이 첫 화면에서 공존
- **Navigation model**: sidebar
- **Density**: balanced
- **Primary regions**:
- **Fact list** (primary): 추출된 사실의 업무 문장 목록과 상태 태그, 초기 필터는 선택 필요+충돌 확인
- **Fact detail and editor** (primary): 선택한 사실의 한 문장 요약과 대상/조건/결과/범위/유효기간 편집, 관계 후보 비교
- **Evidence panel** (secondary): 원문 발췌·매칭 키·추론 규칙·수정 이력, 필요할 때만 여는 우측 패널
- **Review action bar** (primary): 보류/제외/수정 저장/승인, 반영 범위·개수, 승인 불가 사유
- **First-screen contract**:
- 지식 초안 검토 화면은 사실 목록과 선택 사실 편집면을 첫 viewport에 함께 노출한다
- 홈은 통합 검색 입력과 '지식 추가' 주 버튼, 내가 확인할 항목 3종을 장식 요소보다 먼저 배치한다
- 승인 계열 버튼 옆에는 반영 범위와 개수가 항상 보인다
- 그래프 통계·노드 수·Pack 수는 첫 화면에 등장하지 않는다
- **Avoid layouts**:
...
- **Anti-keywords**: graph-tech jargon, decorative node graph, sci-fi glow, generic dashboard metric cards, numeric-score-only ranking, spinner-only progress, marketing hero, paper texture over legibility
- **Tone of voice**: 업무 문장으로 말한다 (기술 어휘 번역), 행동의 결과와 영향 범위를 미리 알려준다, 실패를 숨기지 않고 복구 행동과 함께 보여준다, 차분하고 사무적이되 딱딱하지 않다
- **Visual direction**: prussian blue archive authority, cornsilk warm paper surfaces flattened for reading, ocean blue trust accents for focus and links, hunter green success, goldenrod warning, marsala failure, rust as rare editorial emphasis only, sentence-first fact rows with status tags, split review workbench with evidence panel, long-form korean text at comfortable measure, serif display for knowledge sentences, sans for controls, quiet rules and generous whitespace instead of card walls
- **Interaction direction**: describe knowledge in free sentences with optional help frames, watch a five-stage sentence-based analysis progress, triage extracted facts by needs-choice and conflict first, compare relation candidates as business sentences with reasons, edit a fact under its one-sentence summary, approve with visible scope and count, blocked with visible reason, return to the source text at any time without losing edits, propose a company-specific term without ontology vocabulary, connect two search results and send to review, retry failed intake without duplicating jobs

## 3. Design Principles

- **Archive**: `archive`를 시각적 선택과 인터랙션 선택의 기본 기준으로 삼습니다.
- **Evidence-First**: `evidence-first`를 시각적 선택과 인터랙션 선택의 기본 기준으로 삼습니다.
- **Sentence-First**: `sentence-first`를 시각적 선택과 인터랙션 선택의 기본 기준으로 삼습니다.
...
- **Preferred families**: Deep Blues, Natural Blues, Pastel Yellows, Deep Greens, Standard Yellows, Pantone Trend Reds, Deep Oranges
...
- **Palette roles**:
  ...
  - `structural_support` -> Ocean Blue #4F97A3 / Natural Blues
  - `proof_support` -> Hunter Green #355E3B / Natural Greens
- **Selected colors**:
  - Prussian Blue #003153 / Deep Blues / 저명도, 저채도, 녹색 기가 약하게 섞인 중성 쿨톤 / 고전, 예술, 집중, 권위, 깊이감
  - Ocean Blue #4F97A3 / Natural Blues / 중명도, 중채도, 매트한 청록색 계열 / 신뢰, 정화, 깊이감, 유연함, 안정감
  - Cornsilk #FFF8DC / Pastel Yellows / 고명도, 저채도, 크리미한 옐로 베이스 계열의 톤 / 따뜻함, 부드러움, 내추럴함, 포근함
  - Hunter Green #355E3B / Natural Greens / 저명도, 중고채도의 다크 그린 계열 / 중후함, 신뢰, 클래식, 깊이감, 균형감
  - Goldenrod #DAA520 / Standard Yellows / 중명도, 중채도, 옐로 오렌지 계열 / 따뜻함, 안정감, 고급스러움, 빈티지 감성
  - Marsala #964F4C / Pantone Trend Reds / 적갈색 계열, 와인과 브라운 사이의 중후한 톤 / 성숙함, 안정감, 관능적, 가을/겨울, 클래식
- **Palette candidates**:
  ...
  - assertive-3 (Assertive): primary=Navy Blue, accent=Ochre, surface_tint=Apricot / Navy Blue is inside preferred families.; Ochre matches brand tone keywords.
...
- **Expanded semantic roles**:
  ...
  - `surface_tint` -> Cornsilk #FFF8DC / Pastel Yellows
  - `canvas` -> Canvas #F7F8FA / Runtime Role Policy
  - `surface` -> Surface #FFFFFF / Runtime Role Policy
  - `surface_muted` -> Surface Muted #EEF1F6 / Runtime Role Policy
  - `surface_elevated` -> Surface #FFFFFF / Runtime Role Policy
  - `border` -> Border #D6DDE6 / Runtime Role Policy
  - `border_strong` -> Border Strong #9AA6B2 / Runtime Role Policy
  - `ink` -> Prussian Blue #003153 / Deep Blues
- **Combination lists**:
  - Seed Core: primary=Prussian Blue, accent=Rust, surface_tint=Cornsilk, structural_support=Ocean Blue, proof_support=Hunter Green
  - Surface System: canvas=Canvas, surface=Surface, surface_tint=Cornsilk, border=Border, ink=Prussian Blue, ink_muted=Ink Muted
  - Support Spectrum: support=Apricot, support=Wheat, support=Terracotta, support=Sky Blue, support=Ochre, support=Navy Blue
- **Semantic color selection**: semantic-os-markdown-search-per-run
  - matched pattern: `pattern-brief-palette-coy-mocha-warm-editorial` / COY 2025 모카 무스 warm editorial 웹 팔레트 후보
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=105.43): anchor_background=Mocha Mousse (Pantone COY 2025), depth_support=Blanched Almond, interface_surface=Olive Green, highlight_air=Cornsilk, proof_accent=Greenery (Pantone COY 2017)
  - ontology-quiet-surface-2 (Quiet Surface, score=73.33): anchor_background=Greenery (Pantone COY 2017), depth_support=Olive Green, interface_surface=Kelly Green, highlight_air=Radiant Orchid (Pantone COY 2014), proof_accent=Blue Iris (Pantone COY 2008)
  - ontology-clear-structure-3 (Clear Structure, score=74.92): anchor_background=Radiant Orchid (Pantone COY 2014), depth_support=Byzantium, interface_surface=Greenery (Pantone COY 2017), highlight_air=Grape Compote, proof_accent=Blanched Almond
  - ontology-strong-signal-4 (Strong Signal, score=88.22): anchor_background=Cerulean (Pantone COY 2000), depth_support=Byzantium, interface_surface=Moss Green, highlight_air=Blanched Almond, proof_accent=Greenery (Pantone COY 2017)
  - ontology-cross-family-5 (Cross Family, score=73.62): anchor_background=Blanched Almond, depth_support=Radiant Orchid (Pantone COY 2014), interface_surface=Olive Green, highlight_air=Greenery (Pantone COY 2017), proof_accent=Byzantium
- **Selection rules**:
...
- **Semantic color ontology**: 361 nodes / 1235 edges from semantic-os
- **Matched color keywords**:
  - `primary` -> Prussian Blue #003153 / blue.deep / mood=고전, 예술, 집중 / axes=low_value, low_chroma, cool_bias
  - `accent` -> Rust #B7410E / orange.deep / mood=빈티지, 견고함, 따뜻한 노스탤지어 / axes=low_value, mid_chroma, warm_bias
  - `surface_tint` -> Cornsilk #FFF8DC / yellow.pastel / mood=따뜻함, 부드러움, 내추럴함 / axes=high_value, low_chroma, warm_bias
  - `structural_support` -> Ocean Blue #4F97A3 / blue.natural / mood=신뢰, 정화, 깊이감 / axes=mid_value, mid_chroma
  - `proof_support` -> Hunter Green #355E3B / green.natural / mood=중후함, 신뢰, 클래식 / axes=low_value, high_chroma, mid_chroma
- **Ontology keyword recommendations**:
  - Chili Oil #944537 / orange.pantone_trend / matches 'low_chroma'; matches 'orange'
  - Apricot #FFB27F / orange.natural / matches 'low_chroma'; matches 'orange'
  - Peach Puff #FFDAB9 / orange.pastel / matches 'low_chroma'; matches 'pastel'
  - Salmon #FA8072 / red.pastel / matches 'pastel'; matches '자연스러움'
  - Autumn Blaze #D1933F / yellow.pantone_trend / matches '자연스러움'; matches 'warm'
...
- **Notes**: 정체성 축: Prussian Blue(잉크·권위) + Ocean Blue(신뢰·포커스) + Cornsilk(따뜻한 종이 표면), 지식 작성·검토 화면의 표면은 종이 질감 없이 평면(flat)으로, 본문 가독성이 항상 우선, 역할 고정: 성공=Hunter Green, 경고=Goldenrod, 실패·충돌=Marsala, Rust는 승인·위험 의미로 금지, 드문 편집적 강조(브랜드 포인트)에만 사용, 상태는 색+아이콘+텍스트 3중 인코딩, 색 단독 구분 금지, 다크 모드: 프러시안 심해 표면 위 콘실크 잉크(역전 도면) 방향 유지
...
- **Query seeds**: split review workbench with fact list and detail editor, evidence panel with source excerpt comparison, korean enterprise knowledge intake form calm archive tone
### Visual Direction

- **Density**: airy (confidence 0.35, provenance inferred) / editorial x1.55, hero x0.25, whitespace x0.8
- **Surface Style**: tinted (confidence 0.23, provenance inferred) / warm x0.8, editorial x1.55, paper x0.9
- **Corner Style**: medium (confidence 0.21, provenance inferred) / card x2.75
- **Typography Mood**: editorial (confidence 0.29, provenance inferred) / editorial x1.55, serif x0.8
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Data review surface**: confidence 0.52 / provenance inferred / table x0.9, data x0.9, timeline x0.9, comparison x1.35
- **Split-pane workspace**: confidence 0.48 / provenance inferred / workspace x1.8, editor x2.25, navigation x0.9, topbar x0.9
- **Editorial feed**: confidence 0.42 / provenance inferred / editorial x1.55, review x2.6
- **Dashboard grid**: confidence 0.21 / provenance inferred / dashboard x0.1, metric x0.25, table x0.9
...
- Avoid: decorative node graph as hero
- Avoid: numeric score badges without reasons
- Avoid: spinner-only loading
- Avoid: metric card dashboards
...
- **Flow coverage**:
  ...
  - document: covered (1 context cards; lazyweb)
...
- **Context cards**:
  - `research-query-01` (research-query, planned): split review workbench with fact list and detail editor / flows: data-review, document / morphology: split-pane
  - `research-query-02` (research-query, planned): evidence panel with source excerpt comparison / flows: data-review / morphology: evidence, card-stack
  - `research-query-03` (research-query, planned): korean enterprise knowledge intake form calm archive tone / flows: general-product-ui / morphology: general-interface-composition
- **Research gaps**:
...
- **Product primitives**: global search input, intake method list, free-sentence composer with help frames, upload manifest with trust notice, five-stage sentence progress, failure recovery panel, sentence-first fact row, five-state fact status tag, fact sentence editor with scoped fields, relation candidate comparison card, collapsible evidence panel, source-versus-structured compare view, scoped review action bar, new concept proposal form, link workspace canvas with keyboard path, link verdict card, knowledge lifecycle badge, provenance timeline, impact question buttons, advanced drawer for raw ontology artifacts, operations queue table, quarantine list, sample data badge, empty state guide, left navigation, workspace topbar
- **Decision model**: implementation_basis=llm-authored-component-decision; baseline_policy=coverage-only
- **Authored component decision**: mode=llm-authored; source=brand_profile.component_decision; component_count=24; rationale=화면 명세 v0의 8개 화면과 13절 수용 과업에서 직접 도출. 그래프 캔버스는 보조 표면으로 강등하고, 문장 편집·근거 대조·범위 명시 승인을 핵심 계약으로 고정한다.
- **Implementation families**: navigation, approval, concept-proposal, evidence, fact-review, feedback-and-empty-states, intake-input, knowledge-detail, link-workspace, operations, progress-and-failure, search
...
- **breadcrumbs** (navigation): Hierarchy trail for deep product areas.
- **tabs** (navigation): Peer view switcher for related panels.
- **pagination** (navigation): Paged data navigation for tables and long lists.

- **Rejected/deferred components**:

- **graph-explorer-canvas (primary)** (foundation): 그래프는 지식 상세의 보조 보기와 고급 서랍으로 강등. 주 표면으로 쓰면 온톨로지 어휘 은폐 원칙이 무너진다
- **metric-kpi-card-row** (data-display): 홈에서 그래프 통계·노드 수·Pack 수 노출 금지. 운영 관리자 요약은 운영 상태 화면으로 격리
- **confidence-score-badge** (data-display): 숫자 점수 단독 표시 금지. '왜 이 후보인가' 문장형 근거로 대체
- **wizard-stepper (intake)** (foundation): 직접 설명하기는 자유문장이 원칙. 단계 강제는 명세의 '별도 필드로 강제하지 않는다'와 충돌

- **Advanced component recommendations**:

- No advanced components recommended from this product context.
...
- **Editorial content block** (editorial / 0.39): content-card, featured-story-card, section-header, content-meta, byline-row
- **Data review table** (data-display / 0.37): data-table, column-header, row-actions, filter-toolbar, pagination
- **Conversation sidecar** (overlay / 0.3): chat-panel, message-thread, message-composer, context-drawer

- **navigation**: left-nav, workspace-topbar
- **approval**: review-action-bar
- **concept-proposal**: new-concept-form
- **evidence**: evidence-panel
- **fact-review**: fact-list-row, fact-status-tag, fact-sentence-editor, relation-candidate-card
- **feedback-and-empty-states**: empty-state-guide
- **intake-input**: intake-method-list, describe-composer, upload-manifest-card
- **knowledge-detail**: knowledge-status-badge, provenance-timeline, impact-question-buttons, advanced-drawer
- **link-workspace**: link-workspace-canvas, link-verdict-card
- **operations**: ops-queue-table, quarantine-list
- **progress-and-failure**: analysis-stage-list, analysis-failure-panel
...
- Warning: color_reference.selected_colors canonical name is ambiguous: 'Marsala'; selected exact identity color-keyword-marsala. Use a full label or Semantic ID to choose another identity. Candidates: Marsala (color-keyword-marsala, #964F4C), Marsala (Pantone COY 2015) (color-keyword-pantone-coy-2015-marsala, #955251)
- Warning: [pitfall#3-rebrand] Prussian Blue vs Ocean Blue (blue): lightness diff 31, saturation diff 65 — possible rebrand remnant
- Warning: [pitfall#3-rebrand] Cornsilk vs Goldenrod (yellow): lightness diff 44, saturation diff 26 — possible rebrand remnant
- Warning: visual_reference.sources is empty

## 14. Quick Start

이 문서는 **EKOS Knowledge Intake Design System**의 디자인 시스템 사양입니다.
...
핵심 primitive: **global search input, intake method list, free-sentence composer with help frames, upload manifest with trust notice, five-stage sentence progress**
...
- **Archive**: archive와 충돌하는 컴포넌트 변형은 만들지 않기
- **Evidence-First**: evidence-first와 충돌하는 컴포넌트 변형은 만들지 않기
- **Sentence-First**: sentence-first와 충돌하는 컴포넌트 변형은 만들지 않기
- **Trust Through Consistency**: 일관된 disabled/error/success 패턴
- 모든 시각적 선택에서 **archive, evidence-first, sentence-first** 기준을 적용
...
- **graph-tech jargon** 방향의 디자인 결정을 하지 않음
- **decorative node graph** 방향의 디자인 결정을 하지 않음
- **sci-fi glow** 방향의 디자인 결정을 하지 않음
- **generic dashboard metric cards** 방향의 디자인 결정을 하지 않음
...
:root {
  ...
  --color-structural-support: #4F97A3;
  --color-proof-support: #355E3B;
  ...
  --color-button-ghost-text-default: #475569;
  --color-button-ghost-text-hover: #003153;
  --color-button-ghost-text-disabled: #A8AFB9;
  ...
  --color-input-text-placeholder: #8D96A3;
  --color-input-text-disabled: #A8AFB9;
  ...
  /* --- Nav link --- */
  --color-nav-link-text-default: #475569;
  ...
  /* --- Feedback — info --- */
  --color-feedback-info-surface: #EAF3F4;
  --color-feedback-info-text: #4F97A3;
  --color-feedback-info-border: #81B4BD;
  --color-feedback-info-icon: #4F97A3;

  /* --- Feedback — success --- */
  --color-feedback-success-surface: #E7ECE7;
  --color-feedback-success-text: #355E3B;
  --color-feedback-success-border: #6F8C74;
  --color-feedback-success-icon: #355E3B;
...
|-----------|-------------|
| advanced-drawer | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| analysis-failure-panel | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| analysis-stage-list | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| describe-composer | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| empty-state-guide | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| evidence-panel | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| fact-list-row | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| fact-sentence-editor | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| fact-status-tag | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| global-search-input | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| impact-question-buttons | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| intake-method-list | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| knowledge-status-badge | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| left-nav | `color.Prussian Blue→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| link-verdict-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| link-workspace-canvas | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| new-concept-form | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| ops-queue-table | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| provenance-timeline | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| quarantine-list | `spacing.8→padding`, `radius.none→radius`, `font:Pretendard` |
| relation-candidate-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| review-action-bar | `spacing.12→padding`, `radius.none→radius`, `font:Pretendard` |
| upload-manifest-card | `color.Cornsilk→background`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
| workspace-topbar | `color.Prussian Blue→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Pretendard` |
...
|------------|------------|-------|-------|
| Cornsilk | Ink | 16.76:1 | AAA (pass) |
| Prussian Blue | Paper | 13.43:1 | AAA (pass) |
| Prussian Blue | Cornsilk | 12.60:1 | AAA (pass) |
| Hunter Green | Paper | 7.46:1 | AAA (pass) |
| Cornsilk | Hunter Green | 7.00:1 | AAA (pass) |
| Rust | Paper | 5.56:1 | AA (pass) |
| Ocean Blue | Ink | 5.35:1 | AA (pass) |
| Rust | Cornsilk | 5.22:1 | AA (pass) |
| Prussian Blue | Ocean Blue | 4.02:1 | AA-large (large-only) |
| Ocean Blue | Paper | 3.34:1 | AA-large (large-only) |
| Rust | Ink | 3.21:1 | AA-large (large-only) |
| Cornsilk | Ocean Blue | 3.13:1 | AA-large (large-only) |
| Prussian Blue | Rust | 2.41:1 | fail (FAIL) |
| Hunter Green | Ink | 2.39:1 | fail (FAIL) |
| Ocean Blue | Hunter Green | 2.23:1 | fail (FAIL) |
| Prussian Blue | Hunter Green | 1.80:1 | fail (FAIL) |
| Rust | Ocean Blue | 1.67:1 | fail (FAIL) |
| Rust | Hunter Green | 1.34:1 | fail (FAIL) |
| Prussian Blue | Ink | 1.33:1 | fail (FAIL) |
...
- **intake method list**: —
- **free-sentence composer with help frames**: —
- **upload manifest with trust notice**: —
- **five-stage sentence progress**: —
- **failure recovery panel**: —
- **sentence-first fact row**: —
- **five-state fact status tag**: —
- **relation candidate comparison card**: —
- **collapsible evidence panel**: —
- **source-versus-structured compare view**: —
- **scoped review action bar**: —
- **new concept proposal form**: —
- **link workspace canvas with keyboard path**: —
- **link verdict card**: —
- **knowledge lifecycle badge**: —
- **provenance timeline**: —
- **impact question buttons**: —
- **advanced drawer for raw ontology artifacts**: —
...
- **global search input**: —
- **fact sentence editor with scoped fields**: —
- **command palette**: —
- **forms**: —
- **notifications**: —
...
- **Contract**: `visual-asset-manifest/v2` / preferred manifest `public/generated/design-system/manifest.json`
...
- **Required asset record fields**: `id`, `label`, `slot`, `status`, `asset_path`, `original_png_path`, `format`, `dimensions`, `size_kb`, `sha256`, `intended_for`, `alt_text`, `prompt_summary`, `generation_provenance_version`
...
| Comic panel preview | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Codex image_gen skill | analysis-failure-panel, evidence-panel | `public/generated/design-system/manifest.json` | no API fallback |
| Comic panel preview sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Openverse, Wikimedia Commons, Unsplash, Pexels | analysis-failure-panel, evidence-panel | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Empty-state illustration | generated | supportive-illustration; SVG: allowed when polished, semantic, and not substituting for content media | Codex image_gen skill | empty-state-guide | `public/generated/design-system/manifest.json` | no API fallback |
| Empty-state illustration sourced fallback | sourced | supportive-illustration; SVG: allowed when polished, semantic, and not substituting for content media | Openverse, Wikimedia Commons, Unsplash, Pexels | empty-state-guide | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| evidence panel with source excerpt comparison | Lazyweb MCP real-app corpus | planned | flows: data-review; morphology: evidence, card-stack |
| korean enterprise knowledge intake form calm archive tone | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| split review workbench with fact list and detail editor | Lazyweb MCP real-app corpus | planned | flows: data-review, document; morphology: split-pane |
