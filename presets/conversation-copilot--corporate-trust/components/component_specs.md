...
총 167개 컴포넌트 | 패밀리: button, commerce, copilot-artifact, copilot-chat, data-display, document, editorial, feedback, input, layout, marketing, navigation, overlay, social, workflow
...
- Active visual signals: surface_style=elevated, density=balanced, corner_style=round, top_layout_cue=conversation-panel
...
- **cta_prominence**: CTA prominence는 medium이다. primary action은 분명하게 보이되 화면 전체를 지배하지 않게 유지한다. (source=cards; confidence=0.14; provenance=inferred; direction=raised cards with restrained depth를 기본으로 하고, 균형 잡힌 spacing과 명확한 slot hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=elevated, density=balanced, corner=round, layout=conversation-panel)
...
- **card_elevation_tendency**: 카드는 중간 이하 shadow만 허용하고 deep shadow stack은 피한다. padding은 균형 있게 유지하되 header/body 구획은 분명하게 둔다. (source=cards; confidence=0.14; provenance=inferred; direction=raised cards with restrained depth를 기본으로 하고, 균형 잡힌 spacing과 명확한 slot hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=elevated, density=balanced, corner=round)
- **border_vs_fill_emphasis**: fill 중심이다. depth와 surface contrast가 우선이며 border는 선택적으로만 쓴다. (source=cards; confidence=0.14; provenance=inferred; direction=raised cards with restrained depth를 기본으로 하고, 균형 잡힌 spacing과 명확한 slot hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=elevated, density=balanced, corner=round)
- **chart_panel_framing**: 차트 패널은 single raised surface로 프레이밍하고 nested card는 피한다. 헤더 메타데이터와 본문 visualization 사이의 breathing room을 충분히 둔다. (source=data_display; confidence=0.17; provenance=inferred; direction=정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다.; evidence=layout=data-review-surface, density=balanced, layout=conversation-panel, surface=elevated)
...
- **filter_nav_density**: filter/nav density는 balanced다. global nav와 local filter를 섞지 말고 계층별 간격 차이로 구조를 드러낸다. (source=navigation; confidence=0.29; provenance=inferred; direction=고정 sidebar 또는 split-pane navigation을 우선하고, 현재 위치와 scope를 항상 명시한다.; evidence=Split-pane workspace, layout=conversation-panel, density=balanced)
