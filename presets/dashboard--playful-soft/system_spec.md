...
- **Strategy**:
  - 헤딩(rounded-sans) + 본문(geometric-sans) 조합
  - 한글 서체: Pretendard — 라틴과 x-height/weight 조화
  - 모노스페이스: JetBrains Mono — 코드/데이터 영역 전용
  - calm 키워드 → comfortable spacing, 과한 weight 대비 지양
- **Heading note**: rounded sans, playful-soft 시각 성격 핵심 — workspace-header / section-header / kpi-card label / insight-card title / goal-tracker label, serif/geometric 금지
- **Body note**: data-table cell / filter-chip / activity feed / mood-check 공용 본문, line-height 1.5–1.6 (dashboard soft)
...
- **Loading**: Inter(preload), Nunito(preload), Pretendard(preload), JetBrains Mono(lazy) | display: swap
...
- **Expanded supporting colors**:
  ...
  - Dark Salmon #E9967A / Pastel Oranges / reference-color / Dark Salmon stays inside the preferred families.; Dark Salmon reinforces the brand mood signals.
  - Rose Quartz #F7CAC9 / Pastel Reds / reference-color / Rose Quartz reinforces the brand mood signals.
  - Blanched Almond #FFEBCD / Pastel Yellows / reference-color / Blanched Almond reinforces the brand mood signals.
...
- **Combination lists**:
  ...
  - Support Spectrum: support=Apricot, support=Salmon, support=Dark Salmon, support=Rose Quartz, support=Blanched Almond, support=Wheat
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=142.25): anchor_surface=Forest Green, structural_support=Bottle Green, quiet_background=Celadon, calm_border=Cascade, fresh_accent=Arcadia
  - ontology-quiet-surface-2 (Quiet Surface, score=110.75): anchor_surface=Periwinkle, structural_support=Peach Puff, quiet_background=Cornsilk, calm_border=Lavender Mist, fresh_accent=Creamsicle
  - ontology-clear-structure-3 (Clear Structure, score=109.05): anchor_surface=Periwinkle, structural_support=Hunter Green, quiet_background=Grape Compote, calm_border=Lavender Mist, fresh_accent=Arcadia
  - ontology-strong-signal-4 (Strong Signal, score=112.3): anchor_surface=Hunter Green, structural_support=Forest Green, quiet_background=Buttercream, calm_border=Cascade, fresh_accent=Periwinkle
  - ontology-cross-family-5 (Cross Family, score=115.55): anchor_surface=Bottle Green, structural_support=Chili Oil, quiet_background=Celadon, calm_border=Cascade, fresh_accent=Lemon Yellow
- **Selection rules**:
...
- **Matched color keywords**:
  ...
  - `accent` -> Peach Puff #FFDAB9 / orange.pastel / mood=따뜻함, 부드러움, 순수함 / axes=high_value, low_chroma, warm_bias
  - `surface_tint` -> Mauve #E0B0FF / violet.pastel / mood=낭만적, 세련됨, 회상적 / axes=mid_value, low_chroma, pastel
- **Ontology keyword recommendations**:
  - Salmon #FA8072 / red.pastel / matches 'pastel'; matches '자연스러움'
  - Creamsicle #FFD7A0 / orange.pastel / matches 'low_chroma'; matches 'pastel'
  ...
  - Cornsilk #FFF8DC / yellow.pastel / matches 'low_chroma'; matches 'pastel'
...
- **Density**: dense (confidence 0.52, provenance inferred) / dashboard x4.1000000000000005, table x1.05
- **Surface Style**: tinted (confidence 0.29, provenance inferred) / warm x1.75, editorial x0.15, soft x4.9
- **Corner Style**: round (confidence 0.48, provenance inferred) / rounded x3.1000000000000005, soft x4.9
- **Typography Mood**: utilitarian (confidence 0.5, provenance inferred) / dashboard x4.1000000000000005, admin x2.3, enterprise x0.1
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Dashboard grid**: confidence 0.52 / provenance inferred / dashboard x4.1000000000000005, kpi x1.8, chart x0.9, table x1.05
- **Data review surface**: confidence 0.52 / provenance inferred / table x1.05, grid x1.2, data x0.9, filter x0.9
- **Split-pane workspace**: confidence 0.47 / provenance inferred / workspace x0.9, sidebar x1.8, shell x0.9, navigation x0.9
- **Editorial feed**: confidence 0.21 / provenance inferred / editorial x0.15, feed x0.9, magazine x0.25
...
- **Flow coverage**:
  - dashboard: covered (3 context cards; lazyweb)
  - general-product-ui: covered (2 context cards; lazyweb)
...
- **Context cards**:
  - `research-query-01` (research-query, planned): cal.com booking dashboard / flows: dashboard / morphology: general-interface-composition
  - `research-query-02` (research-query, planned): notion habit tracker / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-03` (research-query, planned): flo wellness dashboard / flows: dashboard / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): finch self care dashboard / flows: dashboard / morphology: general-interface-composition
  - `research-query-05` (research-query, planned): calm meditation admin / flows: settings / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, data-display, editorial, feedback, input, layout, marketing, navigation, workflow, dashboard-growth, dashboard-wellness, overlay, social
...
- **confidence-meter** (feedback, score 6): AI or policy outcome includes uncertainty; users must decide whether to trust, edit, or escalate / pairs with: risk-summary-card, policy-matrix, tool-call-trace
...
- **exception-queue** (data-display, score 6): multiple issues require triage, assignment, and resolution; reviewers need to batch handle exceptions / pairs with: bulk-action-table, policy-matrix, approval-rail
...
- **button**: primary-button, secondary-button, icon-button, form-actions, follow-button, hero-cta-group
...
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, status-badge, empty-feed-illustration, gentle-toast
...
- **navigation**: breadcrumbs, tabs, pagination, operational-rail, filter-bar, app-shell, sidebar-nav, topbar
...
- **overlay**: dialog, popover, tooltip, autocomplete, share-sheet, soft-dialog, chart-tooltip, confirm-dialog
...
| alert-list | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| app-shell | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| autocomplete | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| avatar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| avatar-cluster | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| badge | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| block-controls | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| breadcrumb | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| chart-container | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-legend | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-tooltip | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chat-input | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| checkbox | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| command-palette | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| command-result-item | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-input | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-thread | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| confidence-meter | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| confirm-dialog | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| conversion-funnel | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| customer-logo | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| dashboard-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| data-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| date-picker | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| date-range-picker | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| decision-record-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| dialog | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| empty-feed-illustration | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-state | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| exception-queue | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| experiment-panel | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| feed-item | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| filter-bar | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-builder | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-chip | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-panel | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| follow-button | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-actions | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| gentle-toast | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| goal-grid | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| goal-tracker | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| habit-calendar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| hero-container | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-cta-group | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-eyebrow | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-headline | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-subheadline | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-trust-strip | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-visual | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| icon-button | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-alert | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-format-menu | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inspector-drawer | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-board | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| kanban-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-column | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| logo-cloud | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mention-popup | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-highlight | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-strip | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| mood-chart | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mood-check | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| operational-rail | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| pagination | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| policy-matrix | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| popover | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| post-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| press-quote | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| primary-button | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| profile-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| radio-group | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| reply-composer | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| resizable-split-pane | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| retention-chart | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| risk-summary-card | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| row-actions | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| saved-view-bar | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-field | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-results | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| secondary-button | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-header | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segment-filter | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| segmented-control | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| session-timeline | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| session-tracker | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| share-sheet | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| shortcut-hint | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sidebar-nav | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| slash-command-menu | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| soft-dialog | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-ledger | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| status-badge | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-summary-row | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| step-progress | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| streak-indicator | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| switch | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tabs | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| text-field | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| thread-view | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| ticket-queue | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| time-picker | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| timeline-stream | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| toast | `color.Mauve→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip-guide | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| user-list | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| user-menu | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| wellness-score | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| wizard-layout | `color.Periwinkle→surface`, `color.Peach Puff→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | breadcrumbs, dashboard-card, decision-record-card, kanban-card, post-card, profile-card | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | breadcrumbs, dashboard-card, decision-record-card, kanban-card, post-card, profile-card | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | block-controls, editor-canvas, editor-toolbar, inline-format-menu, press-quote, slash-command-menu | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | block-controls, editor-canvas, editor-toolbar, inline-format-menu, press-quote, slash-command-menu | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| cal.com booking dashboard | Lazyweb MCP real-app corpus | planned | flows: dashboard; morphology: general-interface-composition |
| calm meditation admin | Lazyweb MCP real-app corpus | planned | flows: settings; morphology: general-interface-composition |
| finch self care dashboard | Lazyweb MCP real-app corpus | planned | flows: dashboard; morphology: general-interface-composition |
| flo wellness dashboard | Lazyweb MCP real-app corpus | planned | flows: dashboard; morphology: general-interface-composition |
| headspace session stats | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| notion habit tracker | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
