# Meadow Component Specs

총 134개 컴포넌트 | 패밀리: button, dashboard-growth, dashboard-wellness, data-display, editorial, feedback, input, layout, marketing, navigation, overlay, social, workflow
...
- Active visual signals: surface_style=tinted, density=dense, corner_style=round, top_layout_cue=dashboard-grid
...
- Headline: SUIT | line-height 1.25-1.35 | tracking 0em
...
- SUIT 같은 한글 display 헤딩은 line-height를 1.02 미만으로 낮추지 않는다.
...
- [calm] hover: opacity 변화 (0.08-0.12), elevation 변화 없음
- [calm] motion: 150-200ms ease-out, bounce/spring 없음
- [calm] color: 중성 톤 위주, accent는 최소한으로

### Visual Adaptation Hints

- **cta_prominence**: CTA prominence는 restrained이다. 데이터 작업 흐름을 가리지 않도록 primary만 선명하게 두고 나머지는 text/ghost로 낮춘다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round, layout=dashboard-grid)
...
## dashboard-wellness / dashboard-card

**역할**: consumer wellness admin soft rounded dashboard card — Periwinkle hairline + Mauve hover, sidebar-nav 진입, kpi-card 래퍼, insight-card 컨테이너, dashboard 섹션 wrap, data-table row 연계, filter-chip 필터 컨테이너

**탐지 출처**: wellness habit tracking
...
- **card_elevation_tendency**: 카드는 낮은 elevation만 허용하고 색면 차이로 그룹을 구분한다. 압축된 spacing에서도 header/body/footer 구획은 divider나 tint로 유지한다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round)
- **border_vs_fill_emphasis**: fill 중심이다. tint surface로 성격을 만들고 border는 아주 얇게 보조한다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round)
...
## dashboard-wellness / streak-indicator

**역할**: 연속 habit/wellness streak pill indicator — kpi-card 내 mini, Peach Puff flame + mono days + Periwinkle ring, sidebar-nav 진입 + filter-chip 기간 필터, dashboard-card 단위, data-table row 진입 드릴-다운
...
## dashboard-wellness / habit-calendar

**역할**: habit week×day calendar grid — heat-scale cell (완료 Periwinkle deepest → empty Mauve lightest), today cell Peach Puff ring, sidebar-nav 진입, filter-chip 필터, data-table row 연계, kpi-card 요약, dashboard-card wrapper
...
## dashboard-wellness / wellness-score

**역할**: wellness score gauge/ring — kpi-card 진입, Periwinkle base + Peach Puff highlight + mono score, sidebar-nav 진입, filter-chip 기간 필터, soft radial gradient, data-table row 연계, dashboard-card wrapper
...
## dashboard-wellness / mood-check

**역할**: mood quick-select emoji/color gradient — kpi-card 카드 내, Peach Puff happy → Periwinkle calm → Mauve thoughtful → soft neutral low, sidebar-nav 진입, filter-chip 기간 필터, data-table row 진입
...
## dashboard-wellness / mood-chart

**역할**: mood 시계열 chart — chart-container 안, kpi-card 연계, filter-chip (기간/세그먼트) 필터, sidebar-nav 진입, data-table row 진입, dashboard-card wrapper
...
- **card_elevation_tendency**: 카드는 낮은 elevation만 허용하고 색면 차이로 그룹을 구분한다. 압축된 spacing에서도 header/body/footer 구획은 divider나 tint로 유지한다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round)
- **border_vs_fill_emphasis**: fill 중심이다. tint surface로 성격을 만들고 border는 아주 얇게 보조한다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round)
- **chart_panel_framing**: 차트 패널은 soft tint frame과 restrained divider로 프레이밍한다. 헤더의 metric, controls, plot 영역을 분리하고 내부 여백은 촘촘하게 관리한다. (source=data_display; confidence=0.29; provenance=inferred; direction=정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다.; evidence=layout=dashboard-grid, density=dense, surface=tinted)
...
## dashboard-wellness / session-tracker

**역할**: 명상/수면/운동 session card — dashboard-card 단위, 지속 시간 mono + Peach Puff complete pill + kpi-card 요약, sidebar-nav 진입, filter-chip 필터, data-table row 진입
...
## dashboard-wellness / session-timeline

**역할**: 하루 session timeline — 시간대별 block stack, dashboard-card 안, rounded, kpi-card 요약, sidebar-nav 진입, filter-chip 기간 필터, data-table row 연계
...
## dashboard-wellness / goal-grid

**역할**: 여러 goal-tracker card grid — dashboard-card wrapper, rounded, kpi-card 요약, sidebar-nav 진입, filter-chip 필터, data-table row 연계
...
- **operational_surface_role**: 주 작업 표면은 card wall이 아니라 table/list/rail/canvas 계열로 설계한다. 압축된 표면에서는 row height, column rhythm, sticky toolbar, source/update label을 우선한다. (source=data_display; confidence=0.29; provenance=inferred; direction=정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다.; evidence=layout=dashboard-grid, density=dense)
- **filter_nav_density**: filter/nav density는 compact하다. chip, scope, pagination을 촘촘하게 묶되 의미 단위별 group은 분리한다. (source=navigation; confidence=0.26; provenance=inferred; direction=고정 sidebar 또는 split-pane navigation을 우선하고, 현재 위치와 scope를 항상 명시한다.; evidence=Split-pane workspace, layout=dashboard-grid, density=dense)
...
- [calm] hover: opacity 변화 (0.08-0.12), elevation 변화 없음
- [calm] density: comfortable 모드 기본, 여유로운 padding
- [calm] feedback: subtle inline alert 선호, 과한 컬러 블록 지양
...
- **cta_prominence**: CTA prominence는 strong이다. 섹션당 primary CTA 1개만 fill/accent로 강하게 띄우고 secondary는 조용하게 후퇴시킨다. (source=cards; confidence=0.29; provenance=inferred; direction=low-elevation tinted cards를 기본으로 하고, 압축된 spacing과 얇은 divider 중심의 hierarchy. soft round corner를 기본값으로 유지.; evidence=surface=tinted, density=dense, corner=round, layout=dashboard-grid)
