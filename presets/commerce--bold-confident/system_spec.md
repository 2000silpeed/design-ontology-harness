...
- **Heading note**: geometric sans with impact, drop banner / hero headline / product title / price tag — serif 금지
- **Body note**: product description / cart item / checkout step / filter label 본문 공용, line-height 1.4–1.5 (commerce dense)
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=148.3): masthead_energy=Pure Red, cover_signal=Scarlet, paper_field=Buttercream, feature_frame=Ultra Violet, attention_flash=Illuminating
  - ontology-quiet-surface-2 (Quiet Surface, score=96.6): masthead_energy=Scarlet, cover_signal=Azure Blue, paper_field=Persimmon, feature_frame=Midnight Violet, attention_flash=Lemon Yellow
  - ontology-clear-structure-3 (Clear Structure, score=101.65): masthead_energy=Scarlet, cover_signal=Azure Blue, paper_field=Royal Purple, feature_frame=Grape Compote, attention_flash=Illuminating
  - ontology-strong-signal-4 (Strong Signal, score=111.9): masthead_energy=Pure Orange, cover_signal=Azure Blue, paper_field=Pure Red, feature_frame=Ultra Violet, attention_flash=Ruby
  - ontology-cross-family-5 (Cross Family, score=103.2): masthead_energy=Grenadine, cover_signal=Pure Red, paper_field=Buttercream, feature_frame=Byzantium, attention_flash=Kelly Green
- **Selection rules**:
...
- **Matched color keywords**:
  - `primary` -> Crimson #BD2E4A / red.standard / mood=고급스러움, 강렬함, 감정적 / axes=low_value, mid_chroma
  - `accent` -> Royal Purple #6C3BAA / violet.standard / mood=장엄함, 품격, 권위 / axes=mid_value, high_chroma, warm_bias
...
- **Ontology keyword recommendations**:
  ...
  - Azure Blue #007FFF / blue.standard / matches 'standard'; matches 'high'
...
- **Density**: balanced (confidence 0.42, provenance inferred) / product x10.05
- **Surface Style**: elevated (confidence 0.17, provenance inferred) / modal x1.35
- **Corner Style**: pill (confidence 0.23, provenance inferred) / pill x0.9
- **Typography Mood**: editorial (confidence 0.18, provenance inferred) / editorial x0.25, article x0.15, magazine x0.25
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Narrative landing flow**: confidence 0.49 / provenance inferred / landing x0.15, hero x4.000000000000001
- **Data review surface**: confidence 0.39 / provenance inferred / grid x3.9999999999999996, filter x0.9
- **Split-pane workspace**: confidence 0.24 / provenance inferred / sidebar x0.9, breadcrumb x0.9

### Image-derived Component Hints

- **Cards**: raised cards with restrained depth를 기본으로 하고, 균형 잡힌 spacing과 명확한 slot hierarchy. pill-like actions만 제한적으로 허용. / provenance inferred / surface=elevated, density=balanced, corner=pill
...
- **Data Display**: 정보 밀도를 유지하되 thin dividers와 restrained accent로 hierarchy를 만든다. / provenance inferred / layout=data-review-surface, density=balanced
...
- **Context cards**:
  - `research-query-01` (research-query, planned): nike streetwear drop commerce / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-02` (research-query, planned): supreme product grid / flows: data-review / morphology: dense-table
  - `research-query-03` (research-query, planned): kith product detail bold / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): ssense checkout flow / flows: pricing / morphology: general-interface-composition
  - `research-query-05` (research-query, planned): musinsa mobile commerce / flows: general-product-ui / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, data-display, document, feedback, input, marketing, navigation, commerce, overlay, social
...
- **inspector-drawer** (overlay, score 4): a selected item needs rich detail without leaving the main workflow; users need source facts, owners, versions, or retention metadata / pairs with: policy-matrix, citation-drawer, decision-record-card
...
- **command-palette** (overlay, score 2): the product has many actions or navigation targets; expert users benefit from quick action search / pairs with: shortcut-hint, saved-view-bar, filter-builder
...
- **filter-builder** (input, score 2): users need AND/OR logic across several fields; filters should be saved, shared, or audited / pairs with: saved-view-bar, bulk-action-table, exception-queue
...
- **Data review table** (data-display / 0.44): data-table, column-header, row-actions, filter-toolbar, pagination

- **button**: primary-button, secondary-button, icon-button, add-to-cart-button, wishlist-toggle, form-actions, hero-cta-group, site-nav-cta
- **data-display**: tag, pricing-card, feature-comparison, metric-strip, status-summary-row, task-surface-header, source-ledger, section-header
- **document**: callout, admonition-block, api-reference-table, parameter-table, redline-viewer
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, discount-badge, checkout-step-progress, empty-cart-state
- **input**: text-field, select, checkbox, switch, segmented-control, variant-selector, size-selector, color-swatch-selector
...
- **navigation**: breadcrumbs, tabs, pagination, category-pill, wizard-layout, operational-rail, app-shell, sidebar-nav
...
- **overlay**: dialog, popover, tooltip, quick-view-modal, confirm-dialog, tooltip-guide, autocomplete, chart-tooltip
...
|-----------|-------------|
| add-to-cart-button | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| address-form | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| admonition-block | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| api-reference-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| app-shell | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| autocomplete | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| avatar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| avatar-cluster | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| badge | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumb | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| bulk-action-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| callout | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| cart-drawer | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| cart-item | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| cart-summary | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| category-pill | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-container | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-legend | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-tooltip | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chat-input | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| checkbox | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| checkout-step | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| checkout-step-progress | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| color-swatch-selector | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| column-header | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| command-palette | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-input | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-thread | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| confirm-dialog | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| countdown-timer | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
...
| dialog | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| discount-badge | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| drop-banner | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-cart-state | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-feed-illustration | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-state | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| featured-category-tile | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| feed-item | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| filter-builder | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-chip | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-panel | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-sidebar | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| follow-button | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-column | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-legal | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-link | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-social | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-actions | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| gentle-toast | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-banner | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-container | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-cta-group | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-eyebrow | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-headline | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-subheadline | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-trust-strip | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-visual | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| icon-button | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| image-thumbnail | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inline-alert | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inspector-drawer | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-board | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| kanban-card | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-column | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| lookbook-hero | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mention-popup | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-strip | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| mobile-menu-trigger | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| operational-rail | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| order-summary | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| original-price-strikethrough | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| pagination | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| parameter-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| payment-form | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| popover | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| post-card | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| price-tag | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| pricing-card | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| primary-button | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| product-card | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| product-hero-image | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| profile-card | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| promo-code-input | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| quantity-stepper | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| quick-view-modal | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| radio-group | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| reply-composer | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| row-actions | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| saved-view-bar | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-field | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-results | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| secondary-button | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-header | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segmented-control | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| share-sheet | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sidebar-nav | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-footer | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-header | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-logo | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-nav | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-nav-cta | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| size-selector | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| soft-dialog | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sort-dropdown | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-ledger | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| status-badge | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-summary-row | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| step-progress | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| switch | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tabs | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| text-field | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| toast | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip-guide | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| upgrade-banner | `color.Buttercream→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| user-menu | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| variant-selector | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| version-switcher | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| wishlist-toggle | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| wizard-layout | `color.Crimson→surface`, `color.Royal Purple→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | add-to-cart-button, breadcrumbs, category-pill, color-swatch-selector, cross-sell-grid, discount-badge | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | add-to-cart-button, breadcrumbs, category-pill, color-swatch-selector, cross-sell-grid, discount-badge | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Comic panel preview | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Codex image_gen skill | filter-panel, hero-trust-strip, metric-strip, tabs | `public/generated/design-system/manifest.json` | no API fallback |
...
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | marketing | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | marketing | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Hero image | generated | high-fidelity-raster-support; SVG: False | Codex image_gen skill | feature-comparison, featured-category-tile, hero-banner, hero-container, hero-cta-group, hero-eyebrow | `public/generated/design-system/manifest.json` | no API fallback |
| Hero image sourced fallback | sourced | high-fidelity-raster-support; SVG: False | Openverse, Wikimedia Commons, Unsplash, Pexels | feature-comparison, featured-category-tile, hero-banner, hero-container, hero-cta-group, hero-eyebrow | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| 29cm product detail bold | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| kith product detail bold | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| musinsa mobile commerce | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| nike streetwear drop commerce | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| ssense checkout flow | Lazyweb MCP real-app corpus | planned | flows: pricing; morphology: general-interface-composition |
| supreme product grid | Lazyweb MCP real-app corpus | planned | flows: data-review; morphology: dense-table |
