...
- **Heading note**: editorial serif-ish variable — hero-headline / featured-issue-title / pricing-tier-title / testimonial-quote, reading 감성의 대비 요소
- **Body note**: hero subheadline / pricing description / testimonial body / feature copy, line-height 1.6 reading-first
...
- **Loading**: Inter(preload), Fraunces(preload), Pretendard(preload), JetBrains Mono(lazy) | display: swap
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=133.8): masthead_energy=Pure Red, cover_signal=Scarlet, paper_field=Buttercream, feature_frame=Ultra Violet, attention_flash=Illuminating
  - ontology-quiet-surface-2 (Quiet Surface, score=80.6): masthead_energy=Scarlet, cover_signal=Goldenrod, paper_field=Cornsilk, feature_frame=Ochre Yellow, attention_flash=Buttercream
  - ontology-clear-structure-3 (Clear Structure, score=88.3): masthead_energy=Scarlet, cover_signal=Byzantium, paper_field=Mustard Yellow, feature_frame=Rust, attention_flash=Illuminating
  - ontology-strong-signal-4 (Strong Signal, score=88.9): masthead_energy=Ruby, cover_signal=Amber, paper_field=Chili Oil, feature_frame=Ultra Violet, attention_flash=Burnt Orange
  - ontology-cross-family-5 (Cross Family, score=91.3): masthead_energy=Lemon Yellow, cover_signal=Byzantium, paper_field=Buttercream, feature_frame=Burnt Orange, attention_flash=Olive Green
- **Selection rules**:
...
- **Matched color keywords**:
  - `primary` -> Ochre Yellow #CB9D06 / yellow.deep / mood=따뜻함, 견고함, 전통미 / axes=mid_value, low_value, mid_chroma
  - `accent` -> Rust #B7410E / orange.deep / mood=빈티지, 견고함, 따뜻한 노스탤지어 / axes=low_value, mid_chroma, warm_bias
  - `surface_tint` -> Wheat #F5DEB3 / yellow.natural / mood=따뜻함, 자연스러움, 부드러움 / axes=mid_value, low_chroma, warm_bias
- **Ontology keyword recommendations**:
  - Buttercream #F3E5AB / yellow.pastel / matches 'low_chroma'; matches 'yellow'
  - Copper #B87333 / orange.deep / matches 'orange'; matches 'copper'
  - Apricot #FFB27F / orange.natural / matches 'low_chroma'; matches 'orange'
  - Burnt Orange #CC5500 / orange.deep / matches 'orange'; matches 'warm'
  - Cornsilk #FFF8DC / yellow.pastel / matches 'low_chroma'; matches 'yellow'
...
- **Density**: airy (confidence 0.52, provenance inferred) / editorial x2.05, landing x2.1, hero x3.4
- **Surface Style**: tinted (confidence 0.4, provenance inferred) / warm x5.25, editorial x2.05, cream x1.55
- **Corner Style**: medium (confidence 0.25, provenance inferred) / card x3.9499999999999997
- **Typography Mood**: editorial (confidence 0.4, provenance inferred) / editorial x2.05, magazine x0.1, serif x2.45
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Narrative landing flow**: confidence 0.52 / provenance inferred / landing x2.1, hero x3.4, pricing x4.6000000000000005, testimonial x3.25
- **Data review surface**: confidence 0.44 / provenance inferred / table x2.75, grid x1.05, data x0.15
- **Dashboard grid**: confidence 0.31 / provenance inferred / dashboard x0.1, table x2.75, monitoring x0.15
- **Editorial feed**: confidence 0.31 / provenance inferred / editorial x2.05, magazine x0.1
...
- **Context cards**:
  - `research-query-01` (research-query, planned): stratechery subscribe landing / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-02` (research-query, planned): ghost publisher hero pricing / flows: pricing / morphology: general-interface-composition
  - `research-query-03` (research-query, planned): every newsletter landing / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): substack top publisher landing / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-05` (research-query, planned): the verge newsletter hub / flows: general-product-ui / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, data-display, document, feedback, input, marketing, navigation, commerce, magazine, overlay
...
- **retention-indicator** (feedback, score 2): users need to know whether a record is retained, pending, or expired; policy requires retention visibility near decisions / pairs with: decision-record-card, audit-timeline, inspector-drawer
...
- **Editorial content block** (editorial / 0.41): content-card, featured-story-card, section-header, content-meta, byline-row
- **Review coverage system** (editorial / 0.34): review-card, score-badge, comparison-table, ranking-list, filter-chip

- **button**: primary-button, secondary-button, icon-button, hero-cta-group, form-actions, site-nav-cta, mobile-menu-trigger, player-controls
- **data-display**: pricing-card, feature-comparison, tag, data-table, column-header, row-actions, avatar, profile-card
- **document**: redline-viewer
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, upgrade-banner, status-badge, reading-progress-bar
...
- **navigation**: breadcrumbs, tabs, pagination, app-shell, sidebar-nav, topbar, breadcrumb, tab-bar
...
- **overlay**: dialog, popover, tooltip, user-menu, chart-tooltip, quick-view-modal
...
|-----------|-------------|
| add-to-cart-button | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| app-shell | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| badge | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumb | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| bulk-action-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| category-pill | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-container | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-legend | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chart-tooltip | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| checkbox | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| color-swatch-selector | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| customer-logo | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| data-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| dialog | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| discount-badge | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| drop-banner | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| drop-cap | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| empty-state | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| evidence-graph | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| faq-answer | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| faq-item | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| faq-question | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| faq-section | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| featured-category-tile | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-chip | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-sidebar | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-column | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-legal | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-link | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footer-social | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-actions | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-banner | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-container | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-cta-group | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-eyebrow | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-headline | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-subheadline | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-trust-strip | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| hero-visual | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| icon-button | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| image-thumbnail | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| inline-alert | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| kanban-card | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| logo-cloud | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| lookbook-hero | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| metric-highlight | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mobile-menu-trigger | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| pagination | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| player-controls | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| popover | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| press-quote | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| price-tag | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| pricing-card | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| primary-button | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| product-card | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| product-hero-image | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| profile-card | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| pull-quote | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| quick-view-modal | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| radio-group | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| reading-progress-bar | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| redline-viewer | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| retention-indicator | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| row-actions | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| saved-view-bar | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| secondary-button | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-break | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segmented-control | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sidebar-nav | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-footer | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-header | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-logo | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-nav | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| site-nav-cta | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| size-selector | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| sort-dropdown | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-badge | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| subscription-callout | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| switch | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tabs | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tag | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| testimonial-author | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| testimonial-card | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| testimonial-quote | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| testimonial-section | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| text-field | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| toast | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| upgrade-banner | `color.Wheat→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| user-menu | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| variant-selector | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| video-player | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| volume-slider | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| wishlist-toggle | `color.Ochre Yellow→surface`, `color.Rust→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | add-to-cart-button, article-gallery, breadcrumbs, category-pill, color-swatch-selector, cross-sell-grid | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | add-to-cart-button, article-gallery, breadcrumbs, category-pill, color-swatch-selector, cross-sell-grid | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Comic panel preview | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Codex image_gen skill | hero-trust-strip, tabs | `public/generated/design-system/manifest.json` | no API fallback |
| Comic panel preview sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Openverse, Wikimedia Commons, Unsplash, Pexels | hero-trust-strip, tabs | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | article-gallery, cover-story, drop-cap, feature-article, issue-header, issue-number | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | article-gallery, cover-story, drop-cap, feature-article, issue-header, issue-number | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
| Hero image | generated | high-fidelity-raster-support; SVG: False | Codex image_gen skill | feature-article, feature-comparison, feature-grid-index, featured-category-tile, hero-banner, hero-container | `public/generated/design-system/manifest.json` | no API fallback |
| Hero image sourced fallback | sourced | high-fidelity-raster-support; SVG: False | Openverse, Wikimedia Commons, Unsplash, Pexels | feature-article, feature-comparison, feature-grid-index, featured-category-tile, hero-banner, hero-container | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| every newsletter landing | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| ghost publisher hero pricing | Lazyweb MCP real-app corpus | planned | flows: pricing; morphology: general-interface-composition |
| stratechery subscribe landing | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| substack top publisher landing | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| the verge newsletter hub | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
