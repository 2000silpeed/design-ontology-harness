...
- **Heading note**: geometric sans, 캔버스 도구 chrome 의 정밀한 라벨 — serif 금지
- **Body note**: inspector / layer panel 라벨, property row 본문 공용
...
- **Ontology-searched candidate palettes**:
  - ontology-best-fit-1 (Best Fit, score=125.3): masthead_energy=Pure Red, cover_signal=Scarlet, paper_field=Buttercream, feature_frame=Ultra Violet, attention_flash=Illuminating
  - ontology-quiet-surface-2 (Quiet Surface, score=78.6): masthead_energy=Grenadine, cover_signal=Super Sonic, paper_field=Azure Blue, feature_frame=Byzantium, attention_flash=Kelly Green
  - ontology-clear-structure-3 (Clear Structure, score=84.4): masthead_energy=Goji Berry, cover_signal=Marsala, paper_field=Cascade, feature_frame=Classic Blue, attention_flash=Illuminating
  - ontology-strong-signal-4 (Strong Signal, score=92.4): masthead_energy=Marsala, cover_signal=Cascade, paper_field=Azure Blue, feature_frame=Ultra Violet, attention_flash=Classic Blue
  - ontology-cross-family-5 (Cross Family, score=85.2): masthead_energy=Marsala, cover_signal=Illuminating, paper_field=Buttercream, feature_frame=Grape Compote, attention_flash=Azure Blue
- **Selection rules**:
...
- **Matched color keywords**:
  - `primary` -> Cobalt Violet #804AA8 / violet.standard / mood=안정감, 차분함, 예술적 / axes=mid_value, mid_chroma, cool_bias
  - `accent` -> Amber #FFBF00 / yellow.standard / mood=따뜻함, 세련됨, 고급스러움 / axes=mid_value, mid_chroma, warm_bias
  - `surface_tint` -> Misty Blue #B5C7EB / blue.pastel / mood=차분함, 사색적, 몽환적 / axes=mid_value, low_chroma, cool_bias
- **Ontology keyword recommendations**:
  - Cascade #76C1B1 / green.pantone_trend / matches 'low_chroma'; matches 'with'
  - Chili Oil #944537 / orange.pantone_trend / matches 'low_chroma'; matches 'with'
  - Grape Compote #6B5876 / violet.pantone_trend / matches 'low_chroma'; matches 'with'
  - Ice Blue #D6EAF8 / blue.pastel / matches 'low_chroma'; matches 'with'
  - Naples Yellow #FADA5E / yellow.pastel / matches 'low_chroma'; matches 'with'
...
- **Density**: dense (confidence 0.21, provenance inferred) / dashboard x0.25, control x0.9
- **Surface Style**: flat (confidence 0.18, provenance inferred) / minimal x0.95
- **Corner Style**: medium (confidence 0.13, provenance inferred) / fallback=medium
- **Typography Mood**: utilitarian (confidence 0.19, provenance inferred) / dashboard x0.25, control x0.9
- **Color balance**: temperature=unknown, contrast=unknown, neutral_bias=unknown, provenance=unverified

### Layout Rhythm

- **Split-pane workspace**: confidence 0.38 / provenance inferred / workspace x1.8, editor x0.9, command x0.9
- **Data review surface**: confidence 0.3 / provenance inferred / grid x2.6, data x0.15, timeline x0.15
...
- **Context cards**:
  - `research-query-01` (research-query, planned): figma canvas inspector layer panel / flows: general-product-ui / morphology: card-stack, split-pane
  - `research-query-02` (research-query, planned): framer canvas tool / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-03` (research-query, planned): excalidraw whiteboard / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-04` (research-query, planned): tldraw canvas / flows: general-product-ui / morphology: general-interface-composition
  - `research-query-05` (research-query, planned): rive editor / flows: document / morphology: general-interface-composition
- **Research gaps**:
...
- **Required families**: button, data-display, document, feedback, input, layout, navigation, canvas, foundation, overlay, social, tool-chrome
...
- **inspector-drawer** (overlay, score 8): a selected item needs rich detail without leaving the main workflow; users need source facts, owners, versions, or retention metadata / pairs with: policy-matrix, citation-drawer, decision-record-card
...
- **policy-matrix** (data-display, score 2): multiple policy rules must be checked against multiple claims or fields; reviewers need dense scan-and-drill compliance status / pairs with: risk-summary-card, exception-queue, approval-rail
...
- **Data review table** (data-display / 0.33): data-table, column-header, row-actions, filter-toolbar, pagination
- **Conversation sidecar** (overlay / 0.31): chat-panel, message-thread, message-composer, context-drawer

- **button**: primary-button, secondary-button, icon-button, form-actions, follow-button
- **data-display**: kanban-board, kanban-column, kanban-card, data-table, column-header, row-actions, search-results, tag
- **document**: article-body, table-of-contents, heading-anchor, prose-block, reading-pane, footnote, link-card, api-reference-table
- **feedback**: badge, inline-alert, empty-state, toast, status-dot, shortcut-hint, status-badge, banner
- **input**: text-field, select, checkbox, switch, segmented-control, property-row, format-selector, textarea
- **layout**: resizable-split-pane
- **navigation**: scope-switcher, breadcrumbs, tabs, pagination, app-shell, sidebar-nav, topbar, breadcrumb
...
- **overlay**: command-palette, dialog, popover, tooltip, keyboard-shortcut-cheatsheet, command-result-item, autocomplete, confirm-dialog
...
| app-shell | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| article-body | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| asset-card | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| asset-library | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| autocomplete | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| avatar-cluster | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| badge | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| banner | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumb | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| breadcrumbs | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| bulk-action-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| canvas-workspace | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| chat-input | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| checkbox | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| chip | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| column-header | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| command-palette | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| command-result-item | `spacing.12→padding`, `radius.none→radius`, `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter`, `font:Inter` |
| comment-input | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| comment-thread | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| confirm-dialog | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| contextual-toolbar | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| data-table | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| dialog | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-feed-illustration | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| empty-state | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| evidence-graph | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| export-panel | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| feed-item | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| filter-chip | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| filter-panel | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| follow-button | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| footnote | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| form-actions | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| form-section | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| format-selector | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| gentle-toast | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| grid-overlay | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| heading-anchor | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| icon-button | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inline-alert | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inspector-drawer | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| inspector-panel | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-board | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| kanban-card | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| kanban-column | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| keyboard-shortcut-cheatsheet | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| layer-item | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| layer-panel | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| layer-thumbnail | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| link-card | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| mention-popup | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| metric-strip | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| minimap | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| operational-rail | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| pagination | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| popover | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| post-card | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| prev-next-pager | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| primary-button | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| property-row | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| prose-block | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| radio-group | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| reply-composer | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| ruler | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| scope-switcher | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-field | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| search-results | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| secondary-button | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| section-header | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| segmented-control | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| select | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| selection-handle | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| share-sheet | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| shortcut-hint | `spacing.12→padding`, `radius.none→radius`, `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter`, `font:Inter` |
| sidebar-nav | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| snap-guide | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| soft-dialog | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| source-ledger | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| status-badge | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-dot | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| status-summary-row | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| step-progress | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| switch | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tab-bar | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| table-of-contents | `spacing.8→padding`, `radius.none→radius`, `font:Inter` |
| tabs | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| text-field | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| textarea | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
...
| toast | `color.Misty Blue→background`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| toolbar-group | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
| tooltip | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| tooltip-guide | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| topbar | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| version-switcher | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| wizard-layout | `color.Cobalt Violet→surface`, `color.Amber→emphasis`, `spacing.12→padding`, `radius.md→radius`, `font:Inter` |
| zoom-control | `spacing.12→padding`, `radius.none→radius`, `font:Inter` |
...
| Card thumbnail | generated | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Codex image_gen skill | asset-card, breadcrumbs, kanban-card, layer-thumbnail, link-card, post-card | `public/generated/design-system/manifest.json` | no API fallback |
| Card thumbnail sourced fallback | sourced | content-media; SVG: only for approved product schematics or semantic vector thumbnails | Openverse, Wikimedia Commons, Unsplash, Pexels | asset-card, breadcrumbs, kanban-card, layer-thumbnail, link-card, post-card | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Comic panel preview | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Codex image_gen skill | export-panel, filter-panel, inspector-panel, layer-panel, metric-strip, tabs | `public/generated/design-system/manifest.json` | no API fallback |
| Comic panel preview sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector panel art exists | Openverse, Wikimedia Commons, Unsplash, Pexels | export-panel, filter-panel, inspector-panel, layer-panel, metric-strip, tabs | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
| Editorial cover | generated | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Codex image_gen skill | article-body | `public/generated/design-system/manifest.json` | no API fallback |
| Editorial cover sourced fallback | sourced | high-fidelity-narrative-media; SVG: denied unless approved production vector artwork exists | Openverse, Wikimedia Commons, Unsplash, Pexels | article-body | `public/generated/design-system/manifest.json` | license-verified sourced visual fallback |
...
|---------|----------|------------|-------------|
| excalidraw whiteboard | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| figma canvas inspector layer panel | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: card-stack, split-pane |
| framer canvas tool | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
| rive editor | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
| spline 3d editor | Lazyweb MCP real-app corpus | planned | flows: document; morphology: general-interface-composition |
| tldraw canvas | Lazyweb MCP real-app corpus | planned | flows: general-product-ui; morphology: general-interface-composition |
