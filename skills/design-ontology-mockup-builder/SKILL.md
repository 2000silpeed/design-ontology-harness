...
| DS096 | 엣지 세로 바 장식 — 상태/강조를 컨테이너 가장자리 얇은 바로 표시 (::before 변형 포함) |

**상태 표시 문법:** 안 읽음/선택/강조는 점(dot), 짧은 라벨, 옅은 배경 틴트로 표시한다.
세로 바(border-left든 pseudo-element든)는 문서 문법이며 DS090/DS096이 잡는다.

**구성 아키타입 다양화:** 색·폰트가 달라도 골격이 같으면 같은 앱처럼 읽힌다.
구현 전에 `registry/style_fingerprints.json`에서 최근 프로젝트들의
`separation_style`을 확인하고, 직전 프로젝트와 같은 골격(hairline-rows /
card-wall / whitespace)이면 다른 골격을 선택하거나 선택 이유를 명시한다.
`check-style-divergence`가 직전 프로젝트와 골격이 같을 때 [WARN]을 낸다.
후보 골격: 헤어라인 행 / 여백·스케일 분리 / 다크 인스트루먼트 패널 / 풀블리드
섹션 교차 / 스플릿 패널 / 좌측 러닝 헤드(신문 섹션 헤더) 등.
