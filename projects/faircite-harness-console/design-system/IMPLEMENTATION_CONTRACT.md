...
System: FairCite Evidence Console System
...
- Do not represent evidence maps, relation maps, or product data graphs as hand-positioned HTML nodes connected by rotated CSS lines such as `graph-node`/`graph-edge` or `trace-node`/`trace-line`. Use a real graph/chart library, a semantic SVG/canvas visualization with runtime data, or a ledger/table for simple relationships.
...
- `lint-implementation` promotes ambiguous schematic/mock/placeholder visual surfaces without runtime intent to DS075, media/photo runtime surfaces without assets to DS076, generic initials app marks to DS077, individual media/evidence tiles without assets to DS078, and ad-hoc node-link placeholder graphs to DS082.
