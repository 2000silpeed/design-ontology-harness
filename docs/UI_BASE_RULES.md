...
| `DS108` | 선언된 서체의 로딩 경로 | `@font-face` 또는 웹폰트 URL 존재 | system 서체 스택 |
...
토큰은 파일마다 **가장 가까운 artifact 디렉터리**에서 읽는다. 한 프로젝트에 목업이 여러 개면 각자 자기 `design-system/`을 가질 수 있고, 루트 것만 읽으면 다른 팔레트로 대비비를 판정해서 조용히 틀린 답이 나온다. 디렉터리 이름만 보고 멈추지 않고 `tokens.css`가 실재하는 곳을 찾는다 — 스펙 문서만 들어 있는 `design-system/`이 실제로 존재하기 때문이다.

같은 이유로 artifact 디렉터리는 깊이와 무관하게 린트에서 제외한다. 생성 토큰 파일에는 관리 블록과 원시 hex가 들어 있는 게 정상인데, 구현으로 린트하면 자기 역할 때문에 규칙에 걸린다.
