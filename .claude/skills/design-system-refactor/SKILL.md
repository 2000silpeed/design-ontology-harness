...
먼저 `agent-team/agent-team.json`과 `agent-team/handoffs/`의 최신 인수인계를 읽습니다. UI Implementer가 배정한 범위만 수정하고, 결과는 Team Lead에게 돌려줍니다.

## 실행 절차

### Phase 1: 스펙 로드

1. `agent-team/component_specs.json` 또는 `agent-team/components/component_specs.json`을 읽습니다.
   - 없으면 `agent-team/component_inventory.json`을 읽습니다.
2. `agent-team/token_schema.json`을 읽습니다.
3. `agent-team/system_spec.md`를 읽어 브랜드 키워드, 안티 키워드, 디자인 원칙을 파악합니다.
