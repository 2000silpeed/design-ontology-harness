...
먼저 `agent-team/agent-team.json`과 `agent-team/handoffs/`의 최신 인수인계를 읽습니다. 사용자가 rebuild를 명시적으로 승인한 경우에만 진행하고, 결과는 Team Lead에게 돌려줍니다.
...
1. `agent-team/system_spec.md` — 디자인 원칙, 브랜드 키워드, 안티 키워드
2. `agent-team/token_schema.json` — 토큰 체계 (color, typography, spacing, motion)
3. `agent-team/components/component_specs.json` — 컴포넌트별 상세 스펙
4. `agent-team/component_inventory.json` — 컴포넌트 패밀리
